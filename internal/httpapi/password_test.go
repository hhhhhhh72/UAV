...
func TestPasswordEndpoints(t *testing.T) {
	...
	w := requestAs(t, app, http.MethodPost, "/api/v1/admin/users",
		[]byte(`{"id":"pw-user","role":"individual","password":"InitPass123"}`), "admin-1", domain.RolePlatformAdmin)
	...
	bad := requestAs(t, app, http.MethodPost, "/api/v1/auth/password",
		[]byte(`{"old_password":"wrong-old","new_password":"NewPass456"}`), "pw-user", domain.RoleIndividual)
	if bad.Code != http.StatusBadRequest {
		t.Fatalf("旧密码错误应 400，实际 %d %s", bad.Code, bad.Body.String())
	}
	// 弱口令 → 400
	weak := requestAs(t, app, http.MethodPost, "/api/v1/auth/password",
		[]byte(`{"old_password":"InitPass123","new_password":"short"}`), "pw-user", domain.RoleIndividual)
	...
	ok := requestAs(t, app, http.MethodPost, "/api/v1/auth/password",
		[]byte(`{"old_password":"InitPass123","new_password":"NewPass456"}`), "pw-user", domain.RoleIndividual)
	if ok.Code != http.StatusOK {
		t.Fatalf("改密应 200，实际 %d %s", ok.Code, ok.Body.String())
	}
	// 改密后旧令牌失效（token_version 自增）→ /me 401
	me := requestAs(t, app, http.MethodGet, "/api/v1/me", nil, "pw-user", domain.RoleIndividual)
	...
	if forbidden := requestAs(t, app, http.MethodPost, "/api/v1/admin/users/pw-user/password",
	...
	if rw := requestAs(t, app, http.MethodPost, "/api/v1/admin/users/pw-user/password",
	...
	if rweak := requestAs(t, app, http.MethodPost, "/api/v1/admin/users/pw-user/password",
