...
func (s *Server) createCollege(w http.ResponseWriter, r *http.Request) {
	var in struct{ Name, Region, Description, LogoURL string; Majors, Facilities []string }
	if err := decode(r, &in); err != nil { fail(w, r, http.StatusBadRequest, err); return }
...
func (s *Server) createCooperation(w http.ResponseWriter, r *http.Request) {
	var in struct{ Title, CollegeID, EnterpriseID, CoopType, Description, StartDate, EndDate string; StudentQuota int }
	if err := decode(r, &in); err != nil { fail(w, r, http.StatusBadRequest, err); return }
