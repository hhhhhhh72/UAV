...
func TestResourcePoolRepoCoverage(t *testing.T) {
	...
	p1, err := r.Create(domain.ResourcePool{ID: "pool-1", Name: "应急池", PoolType: "emergency"})
	mbErr(t, "Create", err, false)
	mbStr(t, "Create.ID", p1.ID, "pool-1")
	_, err = r.Create(domain.ResourcePool{ID: "pool-2", Name: "设备池", PoolType: "equipment"})
	mbErr(t, "Create2", err, false)

	f, err := r.FindByID("pool-1")
	mbErr(t, "FindByID", err, false)
	mbStr(t, "FindByID.Name", f.Name, "应急池")
	_, err = r.FindByID("pool-missing")
	...
	emergency, err := r.List("emergency")
	...
	noMatch, err := r.List("team")
	mbErr(t, "List(team)", err, false)
	mbInt(t, "List(team).len", len(noMatch), 0)

	m1, err := r.AddMember(domain.ResourcePoolMember{ID: "m-1", PoolID: "pool-1", ResID: "res-1", ResType: "drone"})
	mbErr(t, "AddMember", err, false)
	mbStr(t, "AddMember.ID", m1.ID, "m-1")
	_, err = r.AddMember(domain.ResourcePoolMember{ID: "m-2", PoolID: "pool-2", ResID: "res-2", ResType: "equipment"})
	mbErr(t, "AddMember2", err, false)

	members, err := r.ListMembers("pool-1")
...
func TestTestSiteRepoCoverage(t *testing.T) {
	...
	_, err := r.Create(domain.TestSite{ID: "site-1", Name: "飞行场", SiteType: "flying_field"})
	mbErr(t, "Create", err, false)
	_, err = r.Create(domain.TestSite{ID: "site-2", Name: "实验室", SiteType: "lab"})
	mbErr(t, "Create2", err, false)

	f, err := r.FindByID("site-1")
	mbErr(t, "FindByID", err, false)
	mbStr(t, "FindByID.Name", f.Name, "飞行场")
	_, err = r.FindByID("site-missing")
	mbErr(t, "FindByID(missing)", err, true)

	all, err := r.List("")
	mbErr(t, "List(all)", err, false)
	mbInt(t, "List(all).len", len(all), 2)
	ff, err := r.List("flying_field")
	mbErr(t, "List(flying_field)", err, false)
	mbInt(t, "List(flying_field).len", len(ff), 1)

	now := time.Now()
	_, err = r.CreateBooking(domain.TestSiteBooking{ID: "bk-1", SiteID: "site-1", UserID: "u-1", CreatedAt: now})
	mbErr(t, "CreateBooking", err, false)
	_, err = r.CreateBooking(domain.TestSiteBooking{ID: "bk-2", SiteID: "site-1", UserID: "u-2", CreatedAt: now.Add(time.Hour)})
	mbErr(t, "CreateBooking2", err, false)
	_, err = r.CreateBooking(domain.TestSiteBooking{ID: "bk-3", SiteID: "site-2", UserID: "u-1", CreatedAt: now.Add(2 * time.Hour)})
	mbErr(t, "CreateBooking3", err, false)

	upd, err := r.UpdateBookingStatus("bk-1", "approved", "通过")
	...
	empty, total3, err := r.ListAllBookings(100, 10)
	...
	us, err := r.UpdateSite(domain.TestSite{ID: "site-1", Name: "飞行场更新", SiteType: "flying_field"})
	mbErr(t, "UpdateSite", err, false)
	mbStr(t, "UpdateSite.Name", us.Name, "飞行场更新")
	_, err = r.UpdateSite(domain.TestSite{ID: "site-missing"})
	mbErr(t, "UpdateSite(missing)", err, true)

	err = r.DeleteSite("site-2")
	mbErr(t, "DeleteSite", err, false)
	err = r.DeleteSite("site-missing")
	mbErr(t, "DeleteSite(missing)", err, true)
	_, err = r.FindByID("site-2")
...
func TestExhibitionRepoCoverage(t *testing.T) {
	...
	e1, err := r.Create(domain.Exhibition{ID: "expo-1", Title: "无人机展", Category: "drone_show"})
	mbErr(t, "Create", err, false)
	...
	f, err := r.FindByID("expo-1")
	mbErr(t, "FindByID", err, false)
	mbStr(t, "FindByID.Title", f.Title, "无人机展")
	_, err = r.FindByID("expo-missing")
	mbErr(t, "FindByID(missing)", err, true)
...
func TestTransformationRepoCoverage(t *testing.T) {
	...
	_, err := r.Create(domain.Transformation{ID: "trans-1", OwnerID: "u-1", Title: "成果转化", Stage: domain.StageLab})
	mbErr(t, "Create", err, false)
	_, err = r.Create(domain.Transformation{ID: "trans-2", OwnerID: "u-2", Title: "成果转化2", Stage: domain.StagePilot})
	mbErr(t, "Create2", err, false)

	f, err := r.FindByID("trans-1")
	mbErr(t, "FindByID", err, false)
	mbStr(t, "FindByID.Title", f.Title, "成果转化")
	_, err = r.FindByID("trans-missing")
	mbErr(t, "FindByID(missing)", err, true)

	all, err := r.List("")
	mbErr(t, "List(all)", err, false)
	mbInt(t, "List(all).len", len(all), 2)
	byOwner, err := r.List("u-1")
	mbErr(t, "List(u-1)", err, false)
	mbInt(t, "List(u-1).len", len(byOwner), 1)
	mbStr(t, "List(u-1)[0].OwnerID", byOwner[0].OwnerID, "u-1")

	ut, err := r.Update(domain.Transformation{ID: "trans-1", OwnerID: "u-1", Title: "updated", Stage: domain.StagePilot})
	mbErr(t, "Update", err, false)
	...
	_, err = r.Update(domain.Transformation{ID: "trans-missing"})
	mbErr(t, "Update(missing)", err, true)

	err = r.Delete("trans-2")
	mbErr(t, "Delete", err, false)
	err = r.Delete("trans-missing")
	mbErr(t, "Delete(missing)", err, true)
	_, err = r.FindByID("trans-2")
	mbErr(t, "FindByID(after delete)", err, true)
}
...
func TestCollegeRepoCoverage(t *testing.T) {
	...
	_, err := r.Create(domain.College{ID: "col-1", Name: "西工大", Region: "陕西"})
	mbErr(t, "Create", err, false)
	_, err = r.Create(domain.College{ID: "col-2", Name: "北航", Region: "北京"})
	mbErr(t, "Create2", err, false)

	f, err := r.FindByID("col-1")
	mbErr(t, "FindByID", err, false)
	mbStr(t, "FindByID.Name", f.Name, "西工大")
	_, err = r.FindByID("col-missing")
	mbErr(t, "FindByID(missing)", err, true)

	all, err := r.List("")
	mbErr(t, "List(all)", err, false)
	mbInt(t, "List(all).len", len(all), 2)
	byRegion, err := r.List("陕西")
	mbErr(t, "List(陕西)", err, false)
	mbInt(t, "List(陕西).len", len(byRegion), 1)
	mbStr(t, "List(陕西)[0].Region", byRegion[0].Region, "陕西")

	uc, err := r.Update(domain.College{ID: "col-1", Name: "西北工业大学", Region: "陕西"})
	mbErr(t, "Update", err, false)
	mbStr(t, "Update.Name", uc.Name, "西北工业大学")
	_, err = r.Update(domain.College{ID: "col-missing"})
	mbErr(t, "Update(missing)", err, true)

	err = r.Delete("col-2")
	mbErr(t, "Delete", err, false)
	err = r.Delete("col-missing")
	mbErr(t, "Delete(missing)", err, true)
	_, err = r.FindByID("col-2")
	mbErr(t, "FindByID(after delete)", err, true)
}
...
func TestStudyTourRepoCoverage(t *testing.T) {
	...
	_, err := r.Create(domain.StudyTour{ID: "tour-1", Title: "研学游"})
	mbErr(t, "Create", err, false)
	_, err = r.Create(domain.StudyTour{ID: "tour-2", Title: "研学游2"})
	mbErr(t, "Create2", err, false)

	f, err := r.FindByID("tour-1")
	mbErr(t, "FindByID", err, false)
	mbStr(t, "FindByID.Title", f.Title, "研学游")
	_, err = r.FindByID("tour-missing")
	mbErr(t, "FindByID(missing)", err, true)
	...
	ut, err := r.Update(domain.StudyTour{ID: "tour-1", Title: "updated"})
	mbErr(t, "Update", err, false)
	mbStr(t, "Update.Title", ut.Title, "updated")
	_, err = r.Update(domain.StudyTour{ID: "tour-missing"})
	mbErr(t, "Update(missing)", err, true)

	err = r.Delete("tour-2")
	mbErr(t, "Delete", err, false)
	err = r.Delete("tour-missing")
	mbErr(t, "Delete(missing)", err, true)
	_, err = r.FindByID("tour-2")
	mbErr(t, "FindByID(after delete)", err, true)
}
...
func TestCooperationRepoCoverage(t *testing.T) {
	...
	_, err := r.Create(domain.CooperationProgram{ID: "coop-1", EnterpriseID: "ent-1", Title: "校企共建"})
	mbErr(t, "Create", err, false)
	_, err = r.Create(domain.CooperationProgram{ID: "coop-2", EnterpriseID: "ent-2", Title: "校企共建2"})
	mbErr(t, "Create2", err, false)

	f, err := r.FindByID("coop-1")
	mbErr(t, "FindByID", err, false)
	mbStr(t, "FindByID.Title", f.Title, "校企共建")
	_, err = r.FindByID("coop-missing")
	mbErr(t, "FindByID(missing)", err, true)

	all, err := r.List("")
	mbErr(t, "List(all)", err, false)
	mbInt(t, "List(all).len", len(all), 2)
	byEnt, err := r.List("ent-1")
	...
	us, err := r.UpdateStatus("coop-1", "active")
	...
	_, err = r.UpdateStatus("coop-missing", "active")
...
func TestRescueCaseRepoCoverage(t *testing.T) {
	...
	_, err := r.Create(domain.RescueCase{ID: "rc-1", Title: "山火救援", EventType: "mountain_fire", Location: "重庆", DroneModel: "M300", TeamName: "蓝天救援队", Summary: "无人机热成像搜索"})
	mbErr(t, "Create", err, false)
	_, err = r.Create(domain.RescueCase{ID: "rc-2", Title: "洪水救援", EventType: "flood", Location: "武汉", DroneModel: "M30", TeamName: "红十字", Summary: "抛投救生圈"})
	mbErr(t, "Create2", err, false)

	f, err := r.FindByID("rc-1")
	mbErr(t, "FindByID", err, false)
	mbStr(t, "FindByID.Title", f.Title, "山火救援")
	_, err = r.FindByID("rc-missing")
	mbErr(t, "FindByID(missing)", err, true)
	...
	byType, total2, err := r.List("mountain_fire", "", 0, 10)
...
func TestEmergencyDeptRepoCoverage(t *testing.T) {
	...
	_, err := r.CreateDept(domain.EmergencyDept{ID: "dept-1", Name: "消防队", DeptType: "fire"})
	mbErr(t, "CreateDept", err, false)
	_, err = r.CreateDept(domain.EmergencyDept{ID: "dept-2", Name: "应急局", DeptType: "emergency_bureau"})
	...
	_, err = r.CreateDrill(domain.EmergencyDrill{ID: "drill-1", DeptID: "dept-1", Title: "联合演练"})
	mbErr(t, "CreateDrill", err, false)
	_, err = r.CreateDrill(domain.EmergencyDrill{ID: "drill-2", DeptID: "dept-2", Title: "联合演练2"})
	mbErr(t, "CreateDrill2", err, false)

	all, err := r.ListDrills("")
	mbErr(t, "ListDrills(all)", err, false)
	mbInt(t, "ListDrills(all).len", len(all), 2)
	byDept, err := r.ListDrills("dept-1")
...
func TestAssociationMemberRepoCoverage(t *testing.T) {
	...
	_, err := r.Create(domain.AssociationMember{ID: "am-1", UserID: "u-1", Role: domain.AssocMember})
	mbErr(t, "Create", err, false)
	_, err = r.Create(domain.AssociationMember{ID: "am-2", UserID: "u-2", Role: domain.AssocPresident})
	mbErr(t, "Create2", err, false)

	f, err := r.FindByUserID("u-1")
	mbErr(t, "FindByUserID", err, false)
	mbStr(t, "FindByUserID.ID", f.ID, "am-1")
	_, err = r.FindByUserID("u-missing")
	...
	byRole, total2, err := r.List(string(domain.AssocPresident), 0, 10)
	...
	ur, err := r.UpdateRole("am-1", domain.AssocVicePresident)
	...
	_, err = r.UpdateRole("am-missing", domain.AssocPresident)
