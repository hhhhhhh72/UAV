...
func TestDemandRepoListAllCoverage(t *testing.T) {
	...
	mustCreate := func(id, publisher, district string, bt domain.BizType, st domain.DemandStatus) {
		_, err := r.Create(domain.Demand{ID: id, PublisherID: publisher, District: district, BizType: bt, Status: st})
...
func TestDemandRepoListByPublisherCoverage(t *testing.T) {
	...
	_, err := r.Create(domain.Demand{ID: "d-pub-9", PublisherID: "enterprise-001", Status: domain.DemandPublished})
	mmErr(t, "demand.Create(ent-001)", err, false)
	_, err = r.Create(domain.Demand{ID: "d-pub-10", PublisherID: "enterprise-002", Status: domain.DemandPublished})
	mmErr(t, "demand.Create(ent-002)", err, false)

	got, err := r.ListByPublisher("enterprise-001")
	mmErr(t, "demand.ListByPublisher", err, false)
	mmInt(t, "demand.ListByPublisher.len", len(got), 2)

	none, err := r.ListByPublisher("nobody")
...
func TestDemandRepoDeleteCoverage(t *testing.T) {
	...
	err := r.Delete("demand-001") // hit seed
	mmErr(t, "demand.Delete(hit)", err, false)
	_, err = r.FindByID("demand-001")
...
func TestDemandRepoCipherCoverage(t *testing.T) {
	...
	created, err := r.Create(domain.Demand{ID: "d-ciph", Contact: "13900000000", PublisherID: "u-ciph", Status: domain.DemandPublished})
...
func TestEnterpriseRepoMainCoverage(t *testing.T) {
	...
	_, _ = r.Create(domain.Enterprise{ID: "e-1", OwnerUserID: "u-1", Name: "一", Status: domain.EnterpriseSubmitted})
	_, _ = r.Create(domain.Enterprise{ID: "e-2", OwnerUserID: "u-2", Name: "二", Status: domain.EnterpriseApproved})
	_, _ = r.Create(domain.Enterprise{ID: "e-3", OwnerUserID: "u-3", Name: "三", Status: domain.EnterpriseSubmitted})

	// ListByStatus 分页：status 过滤 / 空 status 全量 / offset 越界 / limit 截断
	list, total, err := r.ListByStatus("submitted", 0, 10)
	...
	all, total2, err := r.ListByStatus("", 0, 10)
	...
	none, total3, err := r.ListByStatus("submitted", 10, 10)
	...
	page, total4, err := r.ListByStatus("submitted", 0, 1)
...
func TestEnterpriseRepoCipherCoverage(t *testing.T) {
	...
	e, err := r.Create(domain.Enterprise{ID: "ent-ciph", OwnerUserID: "u-1", Name: "加密企业", LicenseURL: "http://lic", AccountName: "账户名", Status: domain.EnterpriseApproved})
...
func TestJobRepoMainCoverage(t *testing.T) {
	...
	_, _ = r.Create(domain.Job{ID: "j-1", EnterpriseID: "ent-1", Title: "A", Status: domain.JobPublished})
	_, _ = r.Create(domain.Job{ID: "j-2", EnterpriseID: "ent-2", Title: "B", Status: domain.JobDraft})

	all, total, err := r.ListAll(0, 10)
	mmErr(t, "job.ListAll", err, false)
	mmInt(t, "job.ListAll.total", total, 2)
	mmInt(t, "job.ListAll.len", len(all), 2)

	none, total2, err := r.ListAll(10, 10)
...
func TestResumeRepoListAllCoverage(t *testing.T) {
	...
	_, _ = r.Create(domain.Resume{ID: "res-1", UserID: "u-1", Title: "A"})
	_, _ = r.Create(domain.Resume{ID: "res-2", UserID: "u-2", Title: "B"})
	...
	none, total2, err := r.ListAll(5, 10)
...
func TestLabourRepoAssignQuoteCoverage(t *testing.T) {
	...
	_, _ = r.CreateAssignment(domain.Assignment{ID: "asgn-1", OrderID: "lo-1", WorkerID: "w-1"})
	_, _ = r.CreateAssignment(domain.Assignment{ID: "asgn-2", OrderID: "lo-1", WorkerID: "w-2"})
	_, _ = r.CreateAssignment(domain.Assignment{ID: "asgn-3", OrderID: "lo-2", WorkerID: "w-1"})

	byOrder, err := r.ListAssignmentsByOrder("lo-1")
	mmErr(t, "labour.ListAssignmentsByOrder", err, false)
	mmInt(t, "labour.ListAssignmentsByOrder.len", len(byOrder), 2)
	byOrderNone, err := r.ListAssignmentsByOrder("nobody")
	mmErr(t, "labour.ListAssignmentsByOrder(empty)", err, false)
	mmInt(t, "labour.ListAssignmentsByOrder(empty).len", len(byOrderNone), 0)

	byWorker, err := r.ListAssignmentsByWorker("w-1")
	mmErr(t, "labour.ListAssignmentsByWorker", err, false)
	mmInt(t, "labour.ListAssignmentsByWorker.len", len(byWorker), 2)
	byWorkerNone, err := r.ListAssignmentsByWorker("nobody")
...
func TestUserRepoCipherCoverage(t *testing.T) {
	...
	u, err := r.Create(domain.User{ID: "u-ciph-1", WechatOpenID: "wx-ciph-1", PhoneCipher: encPhone, Status: "active", Role: domain.RoleIndividual})
	...
	err = r.UpdateProfile("u-ciph-1", domain.UserProfile{Gender: "男", Region: "重庆", Phone: "13900000001"})
	mmErr(t, "user.UpdateProfile(phone)", err, false)
	f2, err := r.FindByID("u-ciph-1")
	...
	// 空 Phone 不修改手机号
	err = r.UpdateProfile("u-ciph-1", domain.UserProfile{Bio: "hello"})
	mmErr(t, "user.UpdateProfile(empty phone)", err, false)
	f3, err := r.FindByID("u-ciph-1")
...
func TestUserRepoNilCipherProfileCoverage(t *testing.T) {
	...
	_, _ = r.Create(domain.User{ID: "u-nil", PhoneCipher: "13800000000", Status: "active"})
	err := r.UpdateProfile("u-nil", domain.UserProfile{Phone: "13700000000"})
	mmErr(t, "user.UpdateProfile(nil cipher)", err, false)
	f, err := r.FindByID("u-nil")
...
func TestApplicationRepoFindByIDCoverage(t *testing.T) {
	...
	_, _ = r.Create(domain.JobApplication{ID: "app-1", JobID: "job-1", ApplicantID: "u-1", Status: domain.AppSubmitted})
	f, err := r.FindByID("app-1")
...
func TestContractTemplateRepoCoverage(t *testing.T) {
	...
	_, err = r.Create(domain.ContractTemplate{ID: "tpl-custom", Name: "自定义", Status: "active"})
...
func TestBidRepoListByBidderCoverage(t *testing.T) {
	...
	_, _ = r.Create(domain.DemandBid{ID: "bid-1", DemandID: "d-1", BidderID: "u-2"})
	_, _ = r.Create(domain.DemandBid{ID: "bid-2", DemandID: "d-2", BidderID: "u-2"})
	_, _ = r.Create(domain.DemandBid{ID: "bid-3", DemandID: "d-3", BidderID: "u-3"})
	got, err := r.ListByBidder("u-2")
	mmErr(t, "bid.ListByBidder", err, false)
	mmInt(t, "bid.ListByBidder.len", len(got), 2)
	none, err := r.ListByBidder("nobody")
...
func TestCertRepoUpdateDeleteCoverage(t *testing.T) {
	...
	_, _ = r.Create(domain.Certificate{ID: "cert-1", UserID: "u-1", Status: "pending"})
	u, err := r.Update(domain.Certificate{ID: "cert-1", UserID: "u-1", Status: "approved", CertNumber: "CN-1"})
	mmErr(t, "cert.Update", err, false)
	mmStr(t, "cert.Update.Status", u.Status, "approved")
	_, err = r.Update(domain.Certificate{ID: "missing"})
	mmErr(t, "cert.Update(miss)", err, true)
	err = r.Delete("cert-1")
	mmErr(t, "cert.Delete(hit)", err, false)
	_, err = r.FindByID("cert-1")
...
func TestPilotRepoUpdateCoverage(t *testing.T) {
	...
	_, _ = r.Create(domain.CertifiedPilot{ID: "pilot-1", UserID: "u-1", RealName: "飞手"})
	u, err := r.Update(domain.CertifiedPilot{ID: "pilot-1", UserID: "u-1", RealName: "updated"})
	mmErr(t, "pilot.Update", err, false)
	mmStr(t, "pilot.Update.RealName", u.RealName, "updated")
	_, err = r.Update(domain.CertifiedPilot{ID: "missing"})
...
func TestServiceListingRepoCoverage(t *testing.T) {
	...
	_, err := r.Create(domain.ServiceListing{ID: "sl-1", ProviderID: "p-1", Title: "巡检服务", Status: "published"})
	mmErr(t, "serviceListing.Create", err, false)
	f, err := r.FindByID("sl-1")
...
func TestRepairRepoListAllCoverage(t *testing.T) {
	...
	_, _ = r.Create(domain.RepairOrder{ID: "rep-1", CustomerID: "u-1"})
	_, _ = r.Create(domain.RepairOrder{ID: "rep-2", CustomerID: "u-2"})
	all, total, err := r.ListAll(0, 10)
...
func TestPolicyRepoListAllCoverage(t *testing.T) {
	...
	_, _ = r.Create(domain.InsurancePolicy{ID: "pol-1", UserID: "u-1"})
	_, _ = r.Create(domain.InsurancePolicy{ID: "pol-2", UserID: "u-2"})
	all, total, err := r.ListAll(0, 10)
...
func TestLoanRepoListAllCoverage(t *testing.T) {
	...
	_, _ = r.Create(domain.LoanApplication{ID: "loan-1", UserID: "u-1"})
	_, _ = r.Create(domain.LoanApplication{ID: "loan-2", UserID: "u-2"})
	all, total, err := r.ListAll(0, 10)
...
func TestMessageRepoCoverage(t *testing.T) {
	...
	_, _ = r.Create(domain.Message{ID: "msg-1", ReceiverID: "u-1", SenderID: "sys"})
	_, _ = r.Create(domain.Message{ID: "msg-2", ReceiverID: "u-1", SenderID: "sys"})
	f, err := r.FindByID("msg-1")
...
func TestEnrollmentRepoCoverage(t *testing.T) {
	...
	_, _ = r.Create(domain.Enrollment{ID: "enr-1", CourseID: "crs-1", UserID: "u-1"})
	_, _ = r.Create(domain.Enrollment{ID: "enr-2", CourseID: "crs-1", UserID: "u-2"})
	u, err := r.Update(domain.Enrollment{ID: "enr-1", CourseID: "crs-1", UserID: "u-1", Status: "approved"})
	mmErr(t, "enrollment.Update", err, false)
	mmStr(t, "enrollment.Update.Status", u.Status, "approved")
	_, err = r.Update(domain.Enrollment{ID: "missing"})
	mmErr(t, "enrollment.Update(miss)", err, true)
	f, err := r.FindByID("enr-1")
...
func TestTradeOrderRepoCoverage(t *testing.T) {
	...
	_, _ = r.Create(domain.TradeOrder{ID: "to-1", BuyerID: "u-1", SellerID: "u-2", Status: "paid"})
	_, _ = r.Create(domain.TradeOrder{ID: "to-2", BuyerID: "u-3", SellerID: "u-4", Status: "paid"})
	u, err := r.UpdateAftersale(domain.TradeOrder{
	...
	_, err = r.UpdateAftersale(domain.TradeOrder{ID: "missing"})
...
func TestEscrowRepoFreezeReleaseRefundCoverage(t *testing.T) {
	...
	// Refund 成功
	_, _ = r.Freeze("u-1", 5000, tx("tx-6", "freeze", 5000))
	_, err = r.Refund("u-1", 5000, tx("tx-7", "refund", 5000))
	mmErr(t, "escrow.Refund", err, false)
	acct2, _ := r.GetAccount("u-1")
	mmInt64(t, "escrow.Refund.FrozenFen", acct2.FrozenFen, 0)
	// Refund 冻结不足
	_, err = r.Refund("u-1", 1000, tx("tx-8", "refund", 1000))
	if !errors.Is(err, repository.ErrInsufficientFrozenBalance) {
