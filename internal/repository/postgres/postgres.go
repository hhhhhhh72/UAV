...
func (r *userRepo) FindByOpenID(openid string) (domain.User, error) {
	var u domain.User
	err := r.pool.QueryRow(context.Background(),
		`SELECT id, wechat_openid, phone_ciphertext, password_hash, role, status, version, created_at, updated_at FROM users WHERE wechat_openid=$1 AND deleted_at IS NULL`, openid).
		Scan(&u.ID, &u.WechatOpenID, &u.PhoneCipher, &u.PasswordHash, &u.Role, &u.Status, &u.Version, &u.CreatedAt, &u.UpdatedAt)
...
func (r *userRepo) Create(u domain.User) (domain.User, error) {
	...
	_, err := r.pool.Exec(context.Background(),
		`INSERT INTO users (id, wechat_openid, phone_ciphertext, password_hash, role, status, version, created_at, updated_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
		u.ID, u.WechatOpenID, u.PhoneCipher, u.PasswordHash, string(u.Role), u.Status, u.Version, u.CreatedAt, u.UpdatedAt)
	return u, err
}
...
func (r *userRepo) FindByID(id string) (domain.User, error) {
	var u domain.User
	err := r.pool.QueryRow(context.Background(),
		`SELECT id, wechat_openid, phone_ciphertext, password_hash, role, status, version, created_at, updated_at FROM users WHERE id=$1 AND deleted_at IS NULL`, id).
		Scan(&u.ID, &u.WechatOpenID, &u.PhoneCipher, &u.PasswordHash, &u.Role, &u.Status, &u.Version, &u.CreatedAt, &u.UpdatedAt)
	if r.cipher != nil && u.PhoneCipher != "" {
		if dec, err := r.cipher.Decrypt(u.PhoneCipher); err == nil {
			u.PhoneCipher = dec
...
func (r *userRepo) All() ([]domain.User, error) {
	rows, err := r.pool.Query(context.Background(), `SELECT id, wechat_openid, phone_ciphertext, password_hash, role, status, version, created_at, updated_at FROM users WHERE deleted_at IS NULL ORDER BY created_at DESC LIMIT 200`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var u domain.User
		if err := rows.Scan(&u.ID, &u.WechatOpenID, &u.PhoneCipher, &u.PasswordHash, &u.Role, &u.Status, &u.Version, &u.CreatedAt, &u.UpdatedAt); err != nil {
			continue
		}
		if r.cipher != nil && u.PhoneCipher != "" {
			if dec, err := r.cipher.Decrypt(u.PhoneCipher); err == nil {
