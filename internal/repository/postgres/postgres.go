...
func (r *jobRepo) ListAll(offset, limit int) ([]domain.Job, int, error) {
	var total int
	r.pool.QueryRow(context.Background(), `SELECT count(*) FROM jobs`).Scan(&total)
...
func (r *pgCommentRepo) ListByPost(postID string) ([]domain.Comment, error) {
	...
	for rows.Next() {
		...
		rows.Scan(&c.ID, &c.PostID, &c.AuthorID, &c.Content, &c.Status, &c.CreatedAt)
		out = append(out, c)
	}
	return out, nil
}
...
func (r *pgReportRepo) ListPending(offset, limit int) ([]domain.Report, int, error) {
	...
	for rows.Next() {
		...
		rows.Scan(&rp.ID, &rp.ReporterID, &rp.ResourceType, &rp.ResourceID, &rp.Reason, &rp.Status, &rp.CreatedAt)
		out = append(out, rp)
	}
	return out, total, nil
}
...
func (r *pgLabourRepo) ListByEmployer(uid string) ([]domain.LabourOrder, error) {
	...
	for rows.Next() {
		var o domain.LabourOrder
		rows.Scan(&o.ID, &o.EmployerID, &o.Title, &o.Description, &o.WorkerCount, &o.StartDate, &o.EndDate, &o.BudgetFen, &o.Status, &o.Version, &o.CreatedAt, &o.UpdatedAt)
		out = append(out, o)
	}
	return out, nil
}
...
func (r *pgLabourRepo) ListQuotes(orderID string) ([]domain.LabourQuote, error) {
	...
	for rows.Next() {
		...
		rows.Scan(&q.ID, &q.OrderID, &q.QuoterID, &q.QuoterName, &q.AmountFen, &q.Proposal, &q.Status, &q.CreatedAt)
		out = append(out, q)
	}
	return out, nil
}
...
func (r *pgExhibitionRepo) List(offset, limit int) ([]domain.Exhibition, int, error) {
	...
	if rows == nil {
		return nil, total, fmt.Errorf("exhibitions query failed")
	}
	defer rows.Close()
