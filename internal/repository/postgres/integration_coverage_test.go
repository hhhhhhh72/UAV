...
// TestCoverage_AssociationMemberRepo 与 containsMemberID 已移除：
// 协会 8 级角色未启用（association_members 0 行、前端零调用），表由迁移 000113 删除。
// Service Listing 独立仓储与表已删除（migration 000110/000111）：服务能力现在是
// prod_type 为服务类目的商品，覆盖见 service/service_listing_merge_test.go。
