...
func databaseURL() string {
	...
	if u == "" { u = "postgres://drone:drone_secret@127.0.0.1:5433/drone_platform?sslmode=disable" }
	return u
}
...
func TestDemandRepo_CreateAndFind(t *testing.T) {
	store := setupStore(t); if store == nil { return }
	...
	if c.ID != id { t.Errorf("ID mismatch") }
	f, err := repo.FindByID(id)
	if err != nil || f.ID != id { t.Fatal("find failed") }
}
...
func TestDemandRepo_SetStatus(t *testing.T) {
	...
	if err != nil || d.Status != domain.DemandPublished { t.Fatalf("set status: %v", err) }
}
...
func TestDemandRepo_CompareAndSetStatus(t *testing.T) {
	...
	if err != nil || !ok || d.Status != domain.DemandPublished { t.Fatal("CAS failed") }
	ok2, _, _ := repo.CompareAndSetStatus(id, domain.DemandCancelled, domain.DemandMatched)
...
func TestBidRepo_CreateAndList(t *testing.T) {
	...
	if len(list) == 0 { t.Fatal("list empty") }
	_ = b
}
...
func TestBidRepo_UpdateStatus(t *testing.T) {
	...
	if err != nil || b.Status != "accepted" { t.Fatalf("update: %v", err) }
}
...
func TestContractRepo_CreateAndUpdateStatus(t *testing.T) {
	...
	if err != nil || c.Status != domain.ContractSent { t.Fatalf("update status: %v", err) }
}
...
func TestContractRepo_FindByID(t *testing.T) {
	...
	if err != nil || c.ID != id { t.Fatal("find failed") }
}
...
func TestEmploymentRepo_ListWithPagination(t *testing.T) {
	...
	if err != nil || total == 0 { t.Fatalf("list: %v total=%d", err, total) }
	_ = list
}
