...
func TestCoverage_ContractTemplateRepo(t *testing.T) {
	...
	id := ug("cov-tpl")
	created, err := r.Create(domain.ContractTemplate{ID: id, Name: "覆盖率模板", Version: 1, Content: "合同正文", Status: "active"})
	if err != nil {
...
func TestCoverage_ContractRepo(t *testing.T) {
	...
	id := ug("cov-contract")
	created, err := r.Create(domain.Contract{ID: id, EnterpriseID: eid, TemplateID: "tpl-001", Status: domain.ContractDraft})
	if err != nil {
	...
	if l, total, err := r.ListByEnterprise(eid, 0, 20); err != nil {
	...
	if l, total, err := r.ListAll(0, 20); err != nil {
		t.Fatalf("ListAll: %v", err)
	} else if total == 0 || !containsByID(l, id, func(v domain.Contract) string { return v.ID }) {
		t.Fatalf("ListAll: contract %s not found (total=%d)", id, total)
	}
	upd, err := r.UpdateStatus(id, domain.ContractSent)
	if err != nil {
	...
	if _, err := r.UpdateStatus(ug("cov-contract-missing"), domain.ContractSent); err == nil {
		t.Fatalf("UpdateStatus: expected not-found error on missing contract")
	}
	if _, err := r.FindByID(ug("cov-contract-missing")); err == nil {
...
func TestCoverage_EmploymentRepo(t *testing.T) {
	...
	eid := ug("cov-emp-ent")
	id := ug("cov-emp")
	created, err := r.Create(domain.EmploymentRequest{ID: id, EnterpriseID: eid, Position: "飞手", Headcount: 3, Status: domain.EmploymentPending})
	if err != nil {
...
func TestCoverage_ResumeRepo(t *testing.T) {
	...
	if l, err := r.ListByUser(uid); err != nil {
	...
	if _, err := r.Update(ug("cov-resume-missing"), got); err == nil {
...
func TestCoverage_JobApplicationRepo(t *testing.T) {
	...
	if _, err := store.NewJobRepository().Create(domain.Job{
	...
	if _, err := store.NewResumeRepository().Create(domain.Resume{
	...
	id := ug("cov-app")
	created, err := r.Create(domain.JobApplication{
	...
	upd, err := r.UpdateStatus(id, domain.AppViewed)
	if err != nil {
		t.Fatalf("UpdateStatus: %v", err)
	}
	...
	if _, err := r.UpdateStatus(ug("cov-app-missing"), domain.AppViewed); err == nil {
		t.Fatalf("UpdateStatus: expected not-found error on missing application")
	}
	if l, err := r.ListByJob(jobID); err != nil {
	...
	if l, err := r.ListByApplicant(userID); err != nil {
...
func TestCoverage_PostRepo(t *testing.T) {
	...
	id := ug("cov-post")
	created, err := r.Create(domain.Post{
	...
	if l, total, err := r.ListPublished(0, 20); err != nil {
	...
	if l, err := r.ListByAuthor(uid); err != nil {
...
func TestCoverage_CommentRepo(t *testing.T) {
	...
	if _, err := store.NewPostRepository().Create(domain.Post{
	...
	id := ug("cov-comment")
	created, err := r.Create(domain.Comment{ID: id, PostID: postID, AuthorID: "cov-u", Content: "评论内容"})
	if err != nil {
	...
	if l, err := r.ListByPost(postID); err != nil {
...
func TestCoverage_ReportRepo(t *testing.T) {
	...
	id := ug("cov-report")
	created, err := r.Create(domain.Report{
	...
	if l, total, err := r.ListPending(0, 20); err != nil {
...
func TestCoverage_ListingRepo(t *testing.T) {
	...
	id := ug("cov-listing")
	created, err := r.Create(domain.Listing{
	...
	if l, total, err := r.ListByStatus("listed", 0, 20); err != nil {
	...
	if err := r.AddFavorite(id, uid); err != nil {
		t.Fatalf("AddFavorite: %v", err)
	}
	if err := r.AddFavorite(id, uid); err != nil {
		t.Fatalf("AddFavorite (idempotent): %v", err)
	}
	if err := r.RemoveFavorite(id, uid); err != nil {
...
func TestCoverage_LabourOrderRepo(t *testing.T) {
	...
	id := ug("cov-labour")
	created, err := r.Create(domain.LabourOrder{
	...
	if l, err := r.ListByEmployer(eid); err != nil {
	...
	qid := ug("cov-quote")
	quote, err := r.CreateQuote(domain.LabourQuote{
	...
	if l, err := r.ListQuotes(id); err != nil {
	...
	aid := ug("cov-assign")
	if _, err := r.CreateAssignment(domain.Assignment{ID: aid, OrderID: id, WorkerID: workerID}); err != nil {
		t.Fatalf("CreateAssignment: %v", err)
	}
	if l, err := r.ListAssignmentsByOrder(id); err != nil {
	...
	if l, err := r.ListAssignmentsByWorker(workerID); err != nil {
...
func TestCoverage_UserRepoCipher(t *testing.T) {
	...
	created, err := r.Create(domain.User{ID: id, WechatOpenID: openid, Name: "加密用户", Status: "active"})
	if err != nil {
	...
	phone := "13900001111"
	if err := r.UpdateProfile(id, domain.UserProfile{Gender: "男", Phone: phone}); err != nil {
	...
	byOpenID, err := r.FindByOpenID(openid)
	if err != nil {
...
func TestCoverage_PolicyRepo(t *testing.T) {
	...
	id := ug("cov-policy")
	created, err := r.Create(domain.InsurancePolicy{
...
func TestCoverage_InspectionRepo(t *testing.T) {
	...
	id := ug("cov-inspect")
	created, err := r.Create(domain.AnnualInspection{
...
func TestCoverage_LoanRepo(t *testing.T) {
	...
	id := ug("cov-loan")
	created, err := r.Create(domain.LoanApplication{
...
func TestCoverage_RepairRepo(t *testing.T) {
	...
	id := ug("cov-repair")
	created, err := r.Create(domain.RepairOrder{
...
func TestCoverage_MessageRepo(t *testing.T) {
	...
	id := ug("cov-msg")
	created, err := r.Create(domain.Message{
	...
	if l, err := r.ListByUser(receiver, false); err != nil {
...
func TestCoverage_IntentRepo(t *testing.T) {
	...
	if _, err := store.NewDemandRepository().Create(domain.Demand{
	...
	id := ug("cov-intent")
	created, err := r.Create(domain.DemandIntent{
	...
	if l, err := r.ListByDemand(demandID); err != nil {
	...
	if l, err := r.ListByIntentor(intentor); err != nil {
		t.Fatalf("ListByIntentor: %v", err)
	} else if !containsByID(l, id, func(v domain.DemandIntent) string { return v.ID }) {
		t.Fatalf("ListByIntentor: intent %s not found", id)
	}
	upd, err := r.UpdateStatus(id, "contacted")
	if err != nil {
		t.Fatalf("UpdateStatus: %v", err)
	}
...
func TestCoverage_WorkOrderRepo(t *testing.T) {
	...
	id := ug("cov-wo")
	created, err := r.Create(domain.WorkOrder{
	...
	if l, err := r.ListByPublisher(publisher); err != nil {
	...
	if l, err := r.ListByWorker(worker); err != nil {
		t.Fatalf("ListByWorker: %v", err)
	} else if !containsByID(l, id, func(v domain.WorkOrder) string { return v.ID }) {
		t.Fatalf("ListByWorker: work order %s not found", id)
	}
	upd, err := r.UpdateStatus(id, domain.WorkOrderOngoing)
	if err != nil {
		t.Fatalf("UpdateStatus: %v", err)
	}
	...
	pupd, err := r.UpdatePhotos(id, []string{"/a.jpg", "/b.jpg"})
	if err != nil {
	...
	cupd, err := r.UpdateCancel(id, "取消原因")
	if err != nil {
...
func TestCoverage_CollegeRepo(t *testing.T) {
	...
	id := ug("cov-college")
	created, err := r.Create(domain.College{
...
func TestCoverage_StudyTourRepo(t *testing.T) {
	...
	id := ug("cov-tour")
	created, err := r.Create(domain.StudyTour{
...
func TestCoverage_ExhibitionRepo(t *testing.T) {
	...
	id := ug("cov-exh")
	created, err := r.Create(domain.Exhibition{
	...
	if l, total, err := r.List(0, 20); err != nil {
		t.Fatalf("List: %v", err)
	...
	bid := ug("cov-booth")
	b, err := r.CreateBooth(domain.ExhibitionBooth{
	...
	if l, err := r.ListBooths(id); err != nil {
	...
	ub, err := r.UpdateBoothStatus(bid, "approved")
	if err != nil {
...
func TestCoverage_TestSiteRepo(t *testing.T) {
	...
	id := ug("cov-site")
	created, err := r.Create(domain.TestSite{
	...
	if l, err := r.List("flying_field"); err != nil {
	...
	bk, err := r.CreateBooking(domain.TestSiteBooking{
	...
	ub, err := r.UpdateBookingStatus(bookID, "approved", "通过")
	if err != nil {
	...
	if l, err := r.ListBookings(id); err != nil {
	...
	if l, err := r.ListBookingsByUser(userID); err != nil {
		t.Fatalf("ListBookingsByUser: %v", err)
	} else if !containsByID(l, bookID, func(v domain.TestSiteBooking) string { return v.ID }) {
		t.Fatalf("ListBookingsByUser: booking %s not found", bookID)
	}
	if l, total, err := r.ListAllBookings(0, 20); err != nil {
	...
	if err := r.DeleteSite(id); err != nil {
...
func TestCoverage_TransformationRepo(t *testing.T) {
	...
	id := ug("cov-trans")
	created, err := r.Create(domain.Transformation{
