...
func TestEscrowRejectsNonPositiveAmounts(t *testing.T) {
	...
	if _, err := svc.Deposit("u", -1); err == nil {
		t.Fatal("negative deposit accepted")
	}
	if _, err := svc.Deposit("u", 0); err == nil {
		t.Fatal("zero deposit accepted")
	}
	if _, err := svc.Freeze("u", -100, "x", "x"); err == nil {
		t.Fatal("negative freeze accepted")
	}
	if _, err := svc.Release("u", "v", -100, "x", "x"); err == nil {
		t.Fatal("negative release accepted")
	}
	if _, err := svc.Refund("u", -100, "x", "x"); err == nil {
...
func TestEscrowFreezeConcurrent(t *testing.T) {
	svc := service.NewEscrowService(memory.NewEscrowRepository())
	if _, err := svc.Deposit("u", 1000); err != nil {
	...
	for i := 0; i < workers; i++ {
		go func() {
			defer wg.Done()
			_, err := svc.Freeze("u", 50, "course", "c-1")
			mu.Lock()
			defer mu.Unlock()
			switch {
			case err == nil:
...
func TestEscrowReleaseAtomicity(t *testing.T) {
	svc := service.NewEscrowService(memory.NewEscrowRepository())
	svc.Deposit("buyer", 1000)
	svc.Freeze("buyer", 800, "course", "c-1")

	// 收款方 seller 从未开过户：Release 应自动创建并正确入账
	if _, err := svc.Release("buyer", "seller", 800, "course", "c-1"); err != nil {
		t.Fatalf("release: %v", err)
	}
	seller, _ := svc.Balance("seller")
	if seller.BalanceFen != 800 {
		t.Fatalf("seller balance: %d, want 800", seller.BalanceFen)
	}
	buyer, _ := svc.Balance("buyer")
	...
	// 冻结不足的解冻必须被拒绝且账户不变
	if _, err := svc.Release("buyer", "seller", 1, "course", "c-1"); !errors.Is(err, repository.ErrInsufficientFrozenBalance) {
		t.Fatalf("expected ErrInsufficientFrozenBalance, got %v", err)
	}
	seller2, _ := svc.Balance("seller")
