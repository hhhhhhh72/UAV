...
func TestComplianceDocCategoryNormalization(t *testing.T) {
	...
	d, err := svc.CreateDoc("适航条例", "policy", "民航局", "2026-01-01", "published", "内容", "", nil)
	if err != nil {
		t.Fatal(err)
	}
	if d.Category != "政策" {
	...
	if d2, _ := svc.CreateDoc("飞行规范", "法规", "民航局", "2026-02-01", "published", "内容", "", nil); d2.Category != "法规" {
		t.Fatalf("stored category %q, want 法规", d2.Category)
	}
	// 英文筛选与中文筛选等价
	for _, cat := range []string{"policy", "政策"} {
		if _, total, err := svc.ListDocs(cat, 1, 20); err != nil || total != 1 {
			t.Errorf("ListDocs(%q): total=%d err=%v, want 1", cat, total, err)
		}
	}
	if _, total, _ := svc.ListDocs("法规", 1, 20); total != 1 {
		t.Errorf("ListDocs(法规): total=%d, want 1", total)
	}
	if _, total, _ := svc.ListDocs("", 1, 20); total != 2 {
...
func TestComplianceStandardCategoryNormalization(t *testing.T) {
	svc := service.NewComplianceService(memory.NewComplianceRepository())
	s, err := svc.CreateStandard("低空飞行标准", "group", "T/CDA-001", "协会", "2026-07-01", "published", "范围", "")
	if err != nil {
		t.Fatal(err)
	}
	if s.Category != "团体标准" {
	...
	if _, err := svc.CreateStandard("国标测试", "national", "GB/T-001", "标委", "2026-06-01", "published", "范围", ""); err != nil {
		t.Fatal(err)
	}
	// 中文/英文筛选等价，各命中各自分类
	for _, cat := range []string{"团体标准", "group"} {
		items, total, err := svc.ListStandards(cat, 1, 20)
		if err != nil || total != 1 {
	...
	if _, total, _ := svc.ListStandards("国家标准", 1, 20); total != 1 {
		t.Errorf("ListStandards(国家标准): total=%d, want 1", total)
	}
	if _, total, _ := svc.ListStandards("", 1, 20); total != 2 {
		t.Errorf("ListStandards(all): total=%d, want 2", total)
	}
	// 更新时同样归一
	upd, err := svc.UpdateStandard(s.ID, "低空飞行标准v2", "enterprise", "T/CDA-002", "协会", "2026-08-01", "范围2", "published", "")
	if err != nil {
		t.Fatal(err)
	}
...
func TestResourceBookingService(t *testing.T) {
	...
	if _, err := svc.Book("u-1", "res-missing", "2026-08-20", "测试", "张三", "13800000000"); !errors.Is(err, service.ErrResourceNotFound) {
		t.Fatalf("book missing resource: err=%v, want ErrResourceNotFound", err)
	}
	res, err := svc.Create("u-admin", "M300 航测机", "drone", "M300", "RTK", "重庆", "预约说明", 50000, "public")
	if err != nil {
		t.Fatal(err)
	}
	bk, err := svc.Book("u-1", res.ID, "2026-08-20", "航测项目", "张三", "13800000000")
	if err != nil {
		t.Fatal(err)
	}
	...
	bookings, err := svc.ListBookingsByResource(res.ID)
