...
func TestCompetitionRegExtendedFields(t *testing.T) {
	...
	reg, err := svc.Register(context.Background(), c.ID, "user-1", "闪电队", 3, "13800000000", "张三", "13800000000",
	...
	// ListRegs 返回同样字段
	regs, err := svc.ListRegs(context.Background(), c.ID)
	...
	// 赛事不存在 → 报错
	if _, err := svc.Register(context.Background(), "comp-nope", "user-1", "闪电队", 3, "138", "", "", "", "", ""); err == nil {
