...
func TestRescueCaseEventTypeNormalization(t *testing.T) {
	...
	// 英文键创建 → 存储中文规范值
	rc, err := svc.Create(context.Background(), "山火案例", "mountain_fire", "南山", "M300", "应急队", "摘要", "结果", "教训", "协会", time.Now())
	if err != nil || rc.EventType != "山火" {
		t.Fatalf("english create: type=%q err=%v, want 山火", rc.EventType, err)
	}
	svc.Create(context.Background(), "洪水案例", "洪水", "嘉陵江", "M300", "应急队", "", "", "", "", time.Now())
	svc.Create(context.Background(), "滑坡案例", "landslide", "武隆", "M300", "应急队", "", "", "", "", time.Now())

	// 中文与英文筛选都能命中中文存量
	if _, total, _ := svc.List(context.Background(), "山火", "", 1, 20); total != 1 {
		t.Fatalf("中文筛选 total=%d, want 1", total)
	}
	if _, total, _ := svc.List(context.Background(), "mountain_fire", "", 1, 20); total != 1 {
		t.Fatalf("英文筛选 total=%d, want 1", total)
	}
	// 未知类型原样传递，仍按精确匹配
	if _, total, _ := svc.List(context.Background(), "landslide", "", 1, 20); total != 1 {
...
func TestRescueCaseListSearch(t *testing.T) {
	svc := service.NewRescueCaseService(memory.NewRescueCaseRepository())
	svc.Create(context.Background(), "南山森林火情侦察", "山火", "重庆市南岸区", "M300", "应急队", "热成像侦察", "", "", "", time.Now())
	svc.Create(context.Background(), "嘉陵江巡查", "洪水", "北碚", "M300", "应急队", "", "", "", "", time.Now())

	if _, total, _ := svc.List(context.Background(), "", "南岸", 1, 20); total != 1 {
		t.Fatalf("q=南岸 total=%d, want 1", total)
	}
	if _, total, _ := svc.List(context.Background(), "", "热成像", 1, 20); total != 1 {
		t.Fatalf("q=热成像 total=%d, want 1", total)
	}
	if _, total, _ := svc.List(context.Background(), "", "不存在", 1, 20); total != 0 {
		t.Fatalf("q=不存在 total=%d, want 0", total)
	}
	// 组合筛选：类型 + q
	if _, total, _ := svc.List(context.Background(), "洪水", "嘉陵江", 1, 20); total != 1 {
		t.Fatalf("组合筛选 total=%d, want 1", total)
	}
	if _, total, _ := svc.List(context.Background(), "山火", "嘉陵江", 1, 20); total != 0 {
...
func TestEmergencyResourceTypeNormalization(t *testing.T) {
	...
	// 中文筛选同样归一
	if _, total, _ := svc.ListResources(context.Background(), "无人机", "", 1, 20); total != 1 {
		t.Fatalf("中文 res_type 筛选 total=%d, want 1", total)
	}
	if _, total, _ := svc.ListResources(context.Background(), "", "热成像", 1, 20); total != 1 {
		t.Fatalf("q=热成像 total=%d, want 1", total)
	}
	if _, total, _ := svc.ListResources(context.Background(), "comm", "指挥车", 1, 20); total != 1 {
		t.Fatalf("组合筛选 total=%d, want 1", total)
	}
}
