...
func TestComplianceService_FindAndDelete(t *testing.T) {
	...
	doc, err := svc.CreateDoc("政策", "政策", "协会", "2026-01-01", "published", "摘要", "http://f", nil)
	if err != nil {
		t.Fatalf("CreateDoc: %v", err)
	}
	// FindDocByID
	if _, err := svc.FindDocByID(doc.ID); err != nil {
		t.Fatalf("FindDocByID: %v", err)
	}
	if _, err := svc.FindDocByID("nope"); err == nil {
		t.Fatal("FindDocByID: expected error for unknown id")
	}

	std, err := svc.CreateStandard("标准", "国家标准", "GB/T", "协会", "2026-01-01", "published", "范围", "http://f")
	if err != nil {
		t.Fatalf("CreateStandard: %v", err)
	}
	// FindStandardByID
	if _, err := svc.FindStandardByID(std.ID); err != nil {
		t.Fatalf("FindStandardByID: %v", err)
	}
	if _, err := svc.FindStandardByID("nope"); err == nil {
		t.Fatal("FindStandardByID: expected error for unknown id")
	}
	// DeleteStandard
	if err := svc.DeleteStandard(std.ID); err != nil {
		t.Fatalf("DeleteStandard: %v", err)
	}
	if err := svc.DeleteStandard("nope"); err == nil {
...
func TestPortfolioService_ListAndDelete(t *testing.T) {
	...
	p, err := svc.Create("ent-1", "品牌", "logo", "cover", "描述", "联系人", nil, nil)
	if err != nil {
...
func TestEnterpriseSvc_AttachDocument(t *testing.T) {
	...
	if _, err := svc.AttachDocument(admin, e.ID, "f2", "idcard"); err != nil {
...
func TestEnterpriseSvc_ListDocuments(t *testing.T) {
	...
	e, _ := svc.Create(owner, service.CreateEnterpriseInput{Name: "测试企业"})
	if _, err := svc.AttachDocument(owner, e.ID, "f1", "license"); err != nil {
		t.Fatalf("AttachDocument: %v", err)
	}

	// not found
	if _, err := svc.ListDocuments(owner, "nope"); err == nil {
		t.Fatal("ListDocuments: expected error for unknown enterprise")
	}
	// 非 owner 非 admin → 403
	if _, err := svc.ListDocuments(domain.Actor{ID: "other", Role: domain.RoleEnterprise}, e.ID); err == nil {
		t.Fatal("ListDocuments: expected permission denied for non-owner non-admin")
	}
	// owner 成功
	docs, err := svc.ListDocuments(owner, e.ID)
	if err != nil || len(docs) != 1 {
		t.Fatalf("ListDocuments(owner): len=%d err=%v", len(docs), err)
	}
	// admin 成功
	docs, err = svc.ListDocuments(domain.Actor{ID: "admin", Role: domain.RolePlatformAdmin}, e.ID)
	if err != nil || len(docs) != 1 {
...
func TestFileService_UploadPaths(t *testing.T) {
	...
	// 正常写入
	rec, err := svc.Upload("owner-1", "a.txt", "text/plain", strings.NewReader("hello"))
	if err != nil {
	...
	// 私有目录
	rec2, err := svc.UploadPrivate("owner-1", "b.txt", "text/plain", strings.NewReader("world"))
	if err != nil {
	...
	// 写失败 → 清理已创建文件并报错
	if _, err := svc.Upload("owner-1", "c.txt", "text/plain", failingReader{}); err == nil {
	...
	if _, err := svcBlock.Upload("o", "f", "text/plain", strings.NewReader("x")); err == nil {
...
func TestHomeService_SortSanitize(t *testing.T) {
	...
	seed := func(id, title string, lat, lng float64) {
		if _, err := demandRepo.Create(domain.Demand{
	...
	if _, err := entRepo.Create(domain.Enterprise{ID: "ent-1", OwnerUserID: "o", Name: "商家", Status: domain.EnterpriseApproved}); err != nil {
		t.Fatal(err)
	}
	svc := service.NewHomeService(demandRepo, entRepo)

	hd := svc.GetHome("重庆", 29.5, 106.5)
...
func TestInsuranceService_ListAllPolicies(t *testing.T) {
	...
	if _, err := svc.CreatePolicy(individualActor(), "M300", "SN", "liability", 100, 10000, time.Now(), time.Now().Add(time.Hour)); err != nil {
		t.Fatalf("CreatePolicy: %v", err)
	}
	list, total, err := svc.ListAllPolicies(0, 10)
	if err != nil || total != 1 || len(list) != 1 {
...
func TestFinanceService_ListAllLoans(t *testing.T) {
	...
	if _, err := svc.ApplyLoan(individualActor(), 100000, 12, "购机"); err != nil {
		t.Fatalf("ApplyLoan: %v", err)
	}
	list, total, err := svc.ListAllLoans(0, 10)
	if err != nil || total != 1 || len(list) != 1 {
...
func TestJobService_ListAllAndDelete(t *testing.T) {
	...
	j, err := svc.CreateJob(entActor(), "飞手", "描述", "重庆", 100000)
	if err != nil {
		t.Fatalf("CreateJob: %v", err)
	}
	list, total, err := svc.ListAllJobs(0, 10)
	if err != nil || total != 1 || len(list) != 1 {
	...
	if err := svc.DeleteJob("nope"); err == nil {
		t.Fatal("DeleteJob: expected error for unknown id")
	}

	if _, err := svc.CreateResume(indActor(), "简历", "张三", "138", "a@b", "本科", "经验", nil, "", "内容", "public"); err != nil {
		t.Fatalf("CreateResume: %v", err)
	}
	res, total, err := svc.ListAllResumes(0, 10)
...
func TestJobService_ListApplicantsForJob(t *testing.T) {
	...
	j, _ := svc.CreateJob(entActor(), "飞手", "描述", "重庆", 100000)

	// 非职位所属企业 → 报错
	if _, err := svc.ListApplicantsForJob(indActor(), j.ID); err == nil {
		t.Fatal("ListApplicantsForJob: expected error for non-owner")
	}
	// 无投递 → 空
	apps, err := svc.ListApplicantsForJob(entActor(), j.ID)
	if err != nil || len(apps) != 0 {
		t.Fatalf("ListApplicantsForJob(empty): len=%d err=%v", len(apps), err)
	}

	// 发布 + 投递 + 简历快照
	if _, err := svc.PublishJob(entActor(), j.ID); err != nil {
		t.Fatalf("PublishJob: %v", err)
	}
	resume, _ := svc.CreateResume(indActor(), "简历", "张三", "138", "a@b", "本科", "经验", nil, "", "内容", "public")
	if _, err := svc.Apply(indActor(), j.ID, resume.ID); err != nil {
		t.Fatalf("Apply: %v", err)
	}
	views, err := svc.ListApplicantsForJob(entActor(), j.ID)
	if err != nil || len(views) != 1 || views[0].Resume.ID != resume.ID {
		t.Fatalf("ListApplicantsForJob: len=%d err=%v", len(views), err)
	}

	// 简历已删 → 跳过该投递
	if _, err := appRepo.Create(domain.JobApplication{
...
func TestLabourService_Assignments(t *testing.T) {
	...
	o, err := svc.CreateOrder(employer, "标题", "描述", 2, time.Now(), time.Now().Add(time.Hour), 100000)
	if err != nil {
		t.Fatalf("CreateOrder: %v", err)
	}

	// CreateAssignment：订单不存在
	if _, err := svc.CreateAssignment(employer, "nope", worker.ID); err == nil {
		t.Fatal("CreateAssignment: expected error for unknown order")
	}
	// 归属校验：非雇主非管理员被拒
	if _, err := svc.CreateAssignment(worker, o.ID, worker.ID); err == nil {
		t.Fatal("CreateAssignment: expected permission denied for non-employer")
	}
	// 雇主成功
	a, err := svc.CreateAssignment(employer, o.ID, worker.ID)
	if err != nil || a.Status != "assigned" {
		t.Fatalf("CreateAssignment(employer): status=%q err=%v", a.Status, err)
	}
	// 管理员成功
	if _, err := svc.CreateAssignment(admin, o.ID, "worker-2"); err != nil {
		t.Fatalf("CreateAssignment(admin): %v", err)
	}

	// ListAssignments：订单不存在
	if _, err := svc.ListAssignments(employer, "nope"); err == nil {
		t.Fatal("ListAssignments: expected error for unknown order")
	}
	// 归属校验
	if _, err := svc.ListAssignments(worker, o.ID); err == nil {
		t.Fatal("ListAssignments: expected permission denied for non-employer")
	}
	// 成功
	list, err := svc.ListAssignments(employer, o.ID)
	if err != nil || len(list) != 2 {
		t.Fatalf("ListAssignments: len=%d err=%v", len(list), err)
	}
	// ListMyAssignments
	mine, err := svc.ListMyAssignments(worker)
...
func TestMessageService_GetListAllDelete(t *testing.T) {
	...
	m, err := svc.Send("s", "r", "标题", "内容", "demand", "d1")
	if err != nil {
		t.Fatalf("Send: %v", err)
	}
	...
	list, total, err := svc.ListAll(0, 10)
	if err != nil || total != 1 || len(list) != 1 {
...
func TestTradeOrderService_AftersaleFlow(t *testing.T) {
	...
	o, err := svc.Create("buyer-1", "p1", "seller-1", 100000)
	if err != nil {
		t.Fatalf("Create: %v", err)
	}
	// 订单不存在
	if _, err := svc.ApplyAftersale("buyer-1", "nope", "refund", "理由", "描述", 100000); err == nil {
		t.Fatal("ApplyAftersale: expected error for unknown order")
	}
	// 非买家 → 拒
	if _, err := svc.ApplyAftersale("seller-1", o.ID, "refund", "", "", 100000); err == nil {
		t.Fatal("ApplyAftersale: expected permission denied for seller")
	}
	// pending → aftersale 非法流转
	if _, err := svc.ApplyAftersale("buyer-1", o.ID, "refund", "", "", 100000); err == nil {
		t.Fatal("ApplyAftersale: expected invalid transition from pending")
	}
	// pending → paid
	if _, err := svc.UpdateStatus(o.ID, "buyer-1", "paid"); err != nil {
		t.Fatalf("UpdateStatus: %v", err)
	}
	// 售后申请成功
	ao, err := svc.ApplyAftersale("buyer-1", o.ID, "refund", "理由", "描述", 100000)
	...
	if _, err := svc.ReviewAftersale("nope", true); err == nil {
		t.Fatal("ReviewAftersale: expected error for unknown order")
	}
	// 同意
	ro, err := svc.ReviewAftersale(o.ID, true)
	...
	// 已结案 → 再次审核报错
	if _, err := svc.ReviewAftersale(o.ID, true); err == nil {
		t.Fatal("ReviewAftersale: expected error when not pending")
	}

	// 驳回分支
	o2, _ := svc.Create("buyer-2", "p2", "seller-2", 200000)
	svc.UpdateStatus(o2.ID, "buyer-2", "paid")
	if _, err := svc.ApplyAftersale("buyer-2", o2.ID, "return", "理由", "描述", 200000); err != nil {
		t.Fatalf("ApplyAftersale(2nd): %v", err)
	}
	rj, err := svc.ReviewAftersale(o2.ID, false)
...
func TestTradeOrderService_AdminDeleteListFind(t *testing.T) {
	...
	o, _ := svc.Create("buyer-1", "p1", "seller-1", 100000)
	// UpdateStatusAdmin：订单不存在
	if _, err := svc.UpdateStatusAdmin("nope", "paid"); err == nil {
		t.Fatal("UpdateStatusAdmin: expected error for unknown order")
	}
	// 非法流转 pending → shipped
	if _, err := svc.UpdateStatusAdmin(o.ID, "shipped"); err == nil {
		t.Fatal("UpdateStatusAdmin: expected invalid transition")
	}
	// 合法流转
	if _, err := svc.UpdateStatusAdmin(o.ID, "paid"); err != nil {
		t.Fatalf("UpdateStatusAdmin: %v", err)
	}
	// FindByID
	if _, err := svc.FindByID(o.ID); err != nil {
		t.Fatalf("FindByID: %v", err)
	}
	if _, err := svc.FindByID("nope"); err == nil {
...
func TestServiceListingService_CRUD(t *testing.T) {
	...
	sl, err := svc.CreateListing("prov-1", "服务商", "巡检服务", "巡检", "描述", "重庆", 10000, "次", "img")
	if err != nil || sl.Status != "published" {
		t.Fatalf("CreateListing: status=%q err=%v", sl.Status, err)
	}
	// 空状态（视为上架）+ 下架 用于 ListPublished 过滤
	if _, err := repo.Create(domain.ServiceListing{ID: "sl-empty", Title: "空状态", Status: ""}); err != nil {
		t.Fatal(err)
	}
	if _, err := repo.Create(domain.ServiceListing{ID: "sl-offline", Title: "下架", Status: "offline"}); err != nil {
		t.Fatal(err)
	}

	lp, err := svc.ListPublished()
	if err != nil || len(lp) != 2 {
		t.Fatalf("ListPublished: len=%d err=%v, want 2 (published + empty)", len(lp), err)
	}

	if _, err := svc.Get(sl.ID); err != nil {
		t.Fatalf("Get: %v", err)
	}
	if _, err := svc.Get("nope"); err == nil {
		t.Fatal("Get: expected error for unknown id")
...
func TestDemandService_AdminOps(t *testing.T) {
	...
	// ListAll 管理端全量
	all, err := svc.ListAll(repository.DemandFilter{})
	if err != nil || len(all) == 0 {
		t.Fatalf("ListAll: len=%d err=%v", len(all), err)
	}

	d, err := svc.Create(pub, service.CreateDemandInput{PublisherName: "企业", Contact: "138", Title: "管理端操作需求"})
	if err != nil {
		t.Fatalf("Create: %v", err)
	}
	// ListByPublisher
	mine, err := svc.ListByPublisher(pub.ID)
	if err != nil || len(mine) != 1 {
		t.Fatalf("ListByPublisher: len=%d err=%v", len(mine), err)
	}

	// CloseByAdmin：非管理员被拒
	if _, err := svc.CloseByAdmin(pub, d.ID, "reason"); err == nil {
		t.Fatal("CloseByAdmin: expected error for non-admin")
	}
	// 未公开（pending）不能关闭
	if _, err := svc.CloseByAdmin(admin, d.ID, "reason"); err == nil {
		t.Fatal("CloseByAdmin: expected error for unpublished demand")
	}
	// 发布后关闭成功
	if _, err := svc.Approve(admin, d.ID); err != nil {
		t.Fatalf("Approve: %v", err)
	}
	closed, err := svc.CloseByAdmin(admin, d.ID, "reason")
	...
	if _, err := svc.SetOfflineAmount(pub, d.ID, 100); err == nil {
		t.Fatal("SetOfflineAmount: expected error for non-admin")
	}
	if _, err := svc.SetOfflineAmount(admin, d.ID, -1); err == nil {
		t.Fatal("SetOfflineAmount: expected error for negative amount")
	}
	d2, _ := svc.Create(pub, service.CreateDemandInput{PublisherName: "企业", Contact: "138", Title: "成交登记需求"})
	if _, err := svc.SetOfflineAmount(admin, d2.ID, 100); err == nil {
		t.Fatal("SetOfflineAmount: expected error for unpublished demand")
	}
	if _, err := svc.Approve(admin, d2.ID); err != nil {
		t.Fatalf("Approve d2: %v", err)
	}
	set, err := svc.SetOfflineAmount(admin, d2.ID, 50000)
	...
	if err := svc.Delete(pub, d2.ID); err == nil {
		t.Fatal("Delete: expected error for non-admin")
	}
	if err := svc.Delete(admin, d2.ID); err == nil {
		t.Fatal("Delete: expected error for published demand")
	}
	if err := svc.Delete(admin, "nope"); err == nil {
		t.Fatal("Delete: expected error for unknown demand")
	}
	d3, _ := svc.Create(pub, service.CreateDemandInput{PublisherName: "企业", Contact: "138", Title: "可删除需求"})
	if _, err := svc.Cancel(pub, d3.ID); err != nil {
		t.Fatalf("Cancel: %v", err)
	}
	if err := svc.Delete(admin, d3.ID); err != nil {
...
func TestTradingService_ProductCRUD(t *testing.T) {
	...
	p, err := svc.CreateProduct(individualActor(), domain.ProductDrone, "无人机", "描述", "品牌", "型号", "new", 100000, nil)
	if err != nil {
		t.Fatalf("CreateProduct: %v", err)
	}
	// GetProduct
	if _, err := svc.GetProduct(p.ID); err != nil {
		t.Fatalf("GetProduct: %v", err)
	}
	if _, err := svc.GetProduct("nope"); err == nil {
		t.Fatal("GetProduct: expected error for unknown id")
	}
	// GetProductAndCountView
	v, err := svc.GetProductAndCountView(p.ID)
	if err != nil || v.Views != 1 {
		t.Fatalf("GetProductAndCountView: views=%d err=%v", v.Views, err)
	}
	if _, err := svc.GetProductAndCountView("nope"); err == nil {
		t.Fatal("GetProductAndCountView: expected error for unknown id")
	}
	// UpdateProduct
	p.Title = "更新"
	up, err := svc.UpdateProduct(p)
	if err != nil || up.Title != "更新" {
		t.Fatalf("UpdateProduct: title=%q err=%v", up.Title, err)
	}
	// DeleteProduct
	if err := svc.DeleteProduct(p.ID); err != nil {
		t.Fatalf("DeleteProduct: %v", err)
	}
	if err := svc.DeleteProduct("nope"); err == nil {
		t.Fatal("DeleteProduct: expected error for unknown id")
	}
	// ListAllRepairs
	if _, err := svc.CreateRepair(individualActor(), "无人机", "故障"); err != nil {
		t.Fatalf("CreateRepair: %v", err)
	}
	list, total, err := svc.ListAllRepairs(0, 10)
	if err != nil || total != 1 || len(list) != 1 {
...
func TestTrainingService_CourseCertCRUD(t *testing.T) {
	...
	c, err := svc.AddCertificate(actor, domain.CertCAAC, "编号", "等级", "机构", time.Now(), time.Now().AddDate(1, 0, 0))
	if err != nil {
		t.Fatalf("AddCertificate: %v", err)
	}
	// GetCert
	if _, err := svc.GetCert(c.ID); err != nil {
		t.Fatalf("GetCert: %v", err)
	}
	if _, err := svc.GetCert("nope"); err == nil {
		t.Fatal("GetCert: expected error for unknown id")
	}
	// UpdateCertificate
	up, err := svc.UpdateCertificate(c.ID, "caac", "新编号", "新等级", "新机构", "approved", time.Now(), time.Now().AddDate(1, 0, 0))
	if err != nil || up.Status != "approved" {
		t.Fatalf("UpdateCertificate: status=%q err=%v", up.Status, err)
	}
	if _, err := svc.UpdateCertificate("nope", "", "", "", "", "", time.Now(), time.Now()); err == nil {
		t.Fatal("UpdateCertificate: expected error for unknown id")
	}
	// DeleteCertificate
	if err := svc.DeleteCertificate(c.ID); err != nil {
		t.Fatalf("DeleteCertificate: %v", err)
	}
	if err := svc.DeleteCertificate("nope"); err == nil {
		t.Fatal("DeleteCertificate: expected error for unknown id")
	}

	// GetCourse
	course, err := svc.CreateCourse(actor, domain.TrainingCourse{Title: "课程"})
	if err != nil {
		t.Fatalf("CreateCourse: %v", err)
	}
	if _, err := svc.GetCourse(course.ID); err != nil {
		t.Fatalf("GetCourse: %v", err)
	}
	if _, err := svc.GetCourse("nope"); err == nil {
		t.Fatal("GetCourse: expected error for unknown id")
	}
	// UpdateCourse
	course.Title = "更新课程"
	upc, err := svc.UpdateCourse(course)
	if err != nil || upc.Title != "更新课程" {
		t.Fatalf("UpdateCourse: title=%q err=%v", upc.Title, err)
	}
	if _, err := svc.UpdateCourse(domain.TrainingCourse{ID: "nope"}); err == nil {
	...
	if err := svc.DeleteCourse("nope"); err == nil {
...
func TestTrainingService_PilotLifecycle(t *testing.T) {
	...
	for i, ct := range types {
		if _, err := certRepo.Create(domain.Certificate{
	...
	p, err := svc.RegisterPilot(actor, "张三", "id", 100, "bio", "avatar", "重庆")
	...
	// 审核中重复申请
	if _, err := svc.RegisterPilot(actor, "张三", "id", 100, "", "", ""); err == nil {
		t.Fatal("RegisterPilot: expected error when pending")
	}

	// RejectPilot：非管理员被拒
	if _, err := svc.RejectPilot(actor, p.ID); err == nil {
		t.Fatal("RejectPilot: expected error for non-admin")
	}
	// 通过认证
	if _, err := svc.ApprovePilot(admin, p.ID); err != nil {
	...
	if _, err := svc.GetPilotDetail("nope"); err == nil {
