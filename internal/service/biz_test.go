...
func TestCaseCRUD(t *testing.T) {
	...
	if _, total, err := svc.List("logistics", 1, 20); err != nil || total != 1 {
		t.Fatalf("list fail")
	}
	_ = c
}
...
func TestComplianceCRUD(t *testing.T) {
	...
	svc.CreateStandard("团体标准", "group", "T/CDA-001", "协会", "2026-07-01", "draft", "标准内容", "")
	if _, total, err := svc.ListDocs("", 1, 20); err != nil || total != 1 {
...
func TestReportCRUD(t *testing.T) {
	...
	if _, total, err := svc.List(1, 20); err != nil || total != 1 {
		t.Fatalf("list fail")
	}
	_ = r
}
...
func TestAchievementCRUD(t *testing.T) {
	...
	if _, total, err := svc.List("无人机", 1, 20); err != nil || total != 1 {
		t.Fatalf("list fail")
	}
	_ = a
}
...
func TestProjectAppCRUD(t *testing.T) {
	...
	if _, err := svc.Review(a.ID, "同意立项", "approve"); err != nil {
		t.Fatal(err)
	}
}
...
func TestCompetitionCRUD(t *testing.T) {
	...
	svc.Register(c.ID, "user-1", "闪电队", 3, "138", "张三", "13800000000", "500101199001011234", "/uploads/photo.jpg", "/uploads/idcard.jpg")
}
