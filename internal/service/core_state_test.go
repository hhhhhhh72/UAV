...
func TestDemandFullLifecycle(t *testing.T) {
	...
	if d.Status != domain.DemandPending { t.Fatalf("expected pending, got %s", d.Status) }

	// Submit for review
	d, err = svc.Submit(entActor(), d.ID)
	if err != nil { t.Fatal(err) }
d, err = svc.Submit(entActor(), d.ID); if err != nil { t.Fatal(err) }

	// Admin approve
	d, err = svc.Approve(admActor(), d.ID)
	if err != nil { t.Fatal(err) }
	if d.Status != domain.DemandPublished { t.Fatalf("expected published, got %s", d.Status) }

	// Create bid
	bid, err := svc.CreateBid(indActor(), d.ID, 50000, "我能做")
	if err != nil { t.Fatal(err) }
	if bid.Status != "pending" { t.Fatalf("bid should be pending, got %s", bid.Status) }

	// Select bid
	d, err = svc.SelectBid(entActor(), d.ID, bid.ID)
	if err != nil { t.Fatal(err) }
	if d.Status != domain.DemandMatched { t.Fatalf("expected matched, got %s", d.Status) }
}

func TestDemandDisputeFlow(t *testing.T) {
	svc := service.NewDemandService(memory.NewDemandRepository(nil), memory.NewBidRepository())
	a := entActor()
	d, _ := svc.Create(a, service.CreateDemandInput{PublisherName: "P", Contact: "138", Title: "T"})
	svc.Submit(a, d.ID)
	svc.Approve(admActor(), d.ID)
	// Non-owner cannot dispute
	if _, err := svc.Dispute(indActor(), d.ID, "test"); err == nil {
		t.Fatal("non-owner should not dispute")
	}
	// Owner can dispute
	d2, err := svc.Dispute(a, d.ID, "不满意")
	if err != nil { t.Fatal(err) }
	_ = d2
}
...
func TestDemandUpdateDraft(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if d2.Title != "新标题" { t.Fatal("title not updated") }
...
func TestContractStateMachine(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if c.Status != domain.ContractDraft { t.Fatalf("expected draft, got %s", c.Status) }
	...
	if err != nil { t.Fatal(err) }
	if c2.Status != domain.ContractSent { t.Fatal("status not sent") }
	// Valid: sent -> signing
	...
	if err != nil { t.Fatal(err) }
	if c5.Status != domain.ContractVoided { t.Fatal("status not voided") }
}
...
func TestEnterpriseReviewFlow(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if e.Status != domain.EnterpriseDraft { t.Fatal("should be draft") }
	// Submit
	e2, err := svc.Submit(a, e.ID)
	if err != nil { t.Fatal(err) }
	if e2.Status != domain.EnterpriseSubmitted { t.Fatal("should be submitted") }
	// Admin review: approve
	e3, err := svc.Review(admActor(), e.ID, "approve", "")
	if err != nil { t.Fatal(err) }
	if e3.Status != domain.EnterpriseApproved { t.Fatal("should be approved") }
}
...
func TestEnterpriseReviewReject(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if e2.Status != domain.EnterpriseRejected { t.Fatal("should be rejected") }
}
...
func TestEscrowDepositFreezeReleaseRefund(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if tx.TxType != "deposit" { t.Fatal("wrong tx type") }
	// Balance
	acct, _ := svc.Balance("user-1")
	if acct.BalanceFen != 100000 { t.Fatalf("balance: %d", acct.BalanceFen) }
	// Freeze
	svc.Freeze("user-1", 30000, "course", "course-1")
	acct2, _ := svc.Balance("user-1")
	if acct2.FrozenFen != 30000 { t.Fatalf("frozen: %d", acct2.FrozenFen) }
	if acct2.BalanceFen != 70000 { t.Fatalf("balance after freeze: %d", acct2.BalanceFen) }
	// Release
	svc.Release("user-1", "org-1", 20000, "course", "course-1")
	acct3, _ := svc.Balance("user-1")
	if acct3.FrozenFen != 10000 { t.Fatalf("frozen after release: %d", acct3.FrozenFen) }
	// Refund
	svc.Refund("user-1", 10000, "course", "course-1")
	acct4, _ := svc.Balance("user-1")
	if acct4.FrozenFen != 0 { t.Fatalf("frozen after refund: %d", acct4.FrozenFen) }
	if acct4.BalanceFen != 80000 { t.Fatalf("balance: %d", acct4.BalanceFen) }
	...
	if len(txs) != 4 { t.Fatalf("expected 4 txs, got %d", len(txs)) }
}
...
func TestMessageSendMarkRead(t *testing.T) {
	...
	if m.IsRead { t.Fatal("should be unread") }
	svc.MarkRead(m.ID)
	// Unread count
	count, _ := svc.UnreadCount("user-1")
	if count != 0 { t.Fatalf("unread: %d", count) }
	// List
	msgs, _ := svc.ListForUser("user-1", false)
	if len(msgs) != 1 { t.Fatal("list all") }
	msgs2, _ := svc.ListForUser("user-1", true)
	if len(msgs2) != 0 { t.Fatal("list unread should be empty") }
}
...
func TestNewsCreatePublish(t *testing.T) {
	...
	if a.Status != "draft" { t.Fatal("should be draft") }
	a2, _ := svc.Publish(a.ID)
	if a2.Status != "published" { t.Fatal("should be published") }
	_, total, _ := svc.ListByCategory("policy", 1, 20)
	if total != 1 { t.Fatal("list by category") }
}
...
func TestTrainingCertCourseFlow(t *testing.T) {
	...
	if cert.Status != "pending" { t.Fatal("should be pending") }
	cert2, _ := svc.ApproveCertificate(admActor(), cert.ID)
	if cert2.Status != "approved" { t.Fatal("should be approved") }
	...
	if course.Status != "draft" { t.Fatal("course should be draft") }
	// Instructor
	...
	if len(certs) != 1 { t.Fatal("list my certs") }
	courses, _ := svc.ListCourses()
	if len(courses) != 1 { t.Fatal("list courses") }
	insts, _ := svc.ListInstructors()
	if len(insts) != 1 { t.Fatal("list instructors") }
	pilots, _ := svc.ListPilots()
	if len(pilots) != 1 { t.Fatal("list pilots") }
}
