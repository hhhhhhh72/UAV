...
func publishDemandForTest(t *testing.T, demandSvc *service.DemandService, pub domain.Actor, title string) domain.Demand {
	t.Helper()
	d, err := demandSvc.Create(pub, service.CreateDemandInput{
		PublisherName: "测试企业", Contact: "13800000000", Title: title,
	})
	if err != nil {
		t.Fatalf("publishDemandForTest create: %v", err)
	}
	if _, err := demandSvc.Approve(domain.Actor{ID: "admin", Role: domain.RolePlatformAdmin}, d.ID); err != nil {
...
func TestResourcePoolService_CRUD(t *testing.T) {
	...
	p, err := svc.Create("无人机资源池", "equipment", "巡检设备池", "owner-1")
	if err != nil {
	...
	list, err := svc.List("equipment")
	if err != nil || len(list) != 1 {
		t.Fatalf("ResourcePoolService.List: len=%d err=%v", len(list), err)
	}
	if empty, _ := svc.List("team"); len(empty) != 0 {
	...
	m, err := svc.AddMember(p.ID, "res-1", "drone", 3)
	if err != nil {
	...
	members, err := svc.ListMembers(p.ID)
	if err != nil || len(members) != 1 {
		t.Fatalf("ResourcePoolService.ListMembers: len=%d err=%v", len(members), err)
	}
	if none, _ := svc.ListMembers("nope"); len(none) != 0 {
...
func TestTestSiteService_CRUD_BookingConflict(t *testing.T) {
	...
	site, err := svc.Create("试飞场", "flying_field", "重庆", "工作日9-18点", "owner-1", 10000, []string{"5G", "RTK"}, "")
	if err != nil {
	...
	if list, _ := svc.List("flying_field"); len(list) != 1 {
	...
	upd, err := svc.UpdateSite(site.ID, "试飞场2", "lab", "南岸", "提前3天", "maintenance", 20000, []string{"RTK"})
	...
	if _, err := svc.UpdateSite("nope", "", "", "", "", "", 0, nil); err == nil {
	...
	bk, err := svc.Book(site.ID, "user-1", "R&D", "张三", "138", start, end)
	if err != nil {
	...
	if _, err := svc.ReviewBooking(bk.ID, "approved", "同意"); err != nil {
		t.Fatalf("TestSiteService.ReviewBooking: %v", err)
	}

	// 与已批准预约重叠 → 冲突报错
	if _, err := svc.Book(site.ID, "user-2", "demo", "李四", "139", start.Add(time.Hour), end.Add(time.Hour)); err == nil {
		t.Fatal("TestSiteService.Book: expected time slot conflict error")
	}
	// 不重叠 → 成功
	if _, err := svc.Book(site.ID, "user-2", "demo", "李四", "139", start.Add(3*time.Hour), end.Add(4*time.Hour)); err != nil {
		t.Fatalf("TestSiteService.Book (non-overlap): %v", err)
	}

	if list, _ := svc.ListBookings(site.ID); len(list) != 2 {
		t.Fatalf("TestSiteService.ListBookings: len=%d, want 2", len(list))
	}
	if _, total, err := svc.ListAllBookings(0, 10); err != nil || total != 2 {
		t.Fatalf("TestSiteService.ListAllBookings: total=%d err=%v", total, err)
	}
	if mine, _ := svc.ListMyBookings("user-1"); len(mine) != 1 {
		t.Fatalf("TestSiteService.ListMyBookings(user-1): len=%d, want 1", len(mine))
	}
	if mine, _ := svc.ListMyBookings("user-2"); len(mine) != 1 {
	...
	if err := svc.DeleteSite("nope"); err == nil {
...
func TestExhibitionService_CRUD_Booths(t *testing.T) {
	...
	e, err := svc.Create("无人机展", "drone_show", "描述", "重庆", "协会", "", start, end, 10, 5000, "")
	if err != nil {
	...
	if _, total, err := svc.List(1, 10); err != nil || total != 1 {
	...
	upd, err := svc.Update(e.ID, "无人机展2", "equipment_expo", "新描述", "北京", "主办", start, end, 20, 8000, "recruiting")
	...
	if _, err := svc.Update("nope", "", "", "", "", "", start, end, 0, 0, ""); err == nil {
		t.Fatal("ExhibitionService.Update: expected error for unknown id")
	}

	b, err := svc.ApplyBooth(e.ID, "ent-1", "A1", "展品", "展品说明")
	if err != nil {
...
func TestTransformationService_CRUD_StageMilestone(t *testing.T) {
	...
	tr, err := svc.Create("成果转化", "ach-1", "owner-1", "partner-1")
	if err != nil {
	...
	if _, err := svc.UpdateTrans("nope", "", "", "", "", ""); err == nil {
		t.Fatal("TransformationService.UpdateTrans: expected error for unknown id")
	}

	// AdvanceStage：不存在的转化 → 报错
	if _, err := svc.AdvanceStage(owner, "nope", domain.StagePilot, ""); err == nil {
		t.Fatal("TransformationService.AdvanceStage: expected error for unknown id")
	}
	// AdvanceStage：负责人可推进
	adv, err := svc.AdvanceStage(owner, tr.ID, domain.StageIndustrialized, "产业化")
	...
	// 非负责人/非管理员 → 报错
	if _, err := svc.AdvanceStage(domain.Actor{ID: "other", Role: domain.RoleEnterprise}, tr.ID, domain.StageListed, ""); err == nil {
		t.Fatal("TransformationService.AdvanceStage: expected error for non-owner")
	}
	// 管理员可推进
	adv2, err := svc.AdvanceStage(admin, tr.ID, domain.StageListed, "上市")
	...
	if _, err := svc.AddMilestone(owner, "nope", "里程碑", ""); err == nil {
		t.Fatal("TransformationService.AddMilestone: expected error for unknown id")
	}
	// AddMilestone：负责人可添加
	m, err := svc.AddMilestone(owner, tr.ID, "里程碑1", "证据URL")
	...
	// 非负责人 → 报错
	if _, err := svc.AddMilestone(domain.Actor{ID: "other", Role: domain.RoleEnterprise}, tr.ID, "里程碑2", ""); err == nil {
		t.Fatal("TransformationService.AddMilestone: expected error for non-owner")
	}

	if err := svc.DeleteTrans(tr.ID); err != nil {
		t.Fatalf("TransformationService.DeleteTrans: %v", err)
	}
	if err := svc.DeleteTrans("nope"); err == nil {
...
func TestCollegeService_CRUD(t *testing.T) {
	...
	c, err := svc.Create(domain.College{Name: "重庆大学", Region: "重庆", City: "重庆"})
	if err != nil {
	...
	if list, _ := svc.List("重庆"); len(list) != 1 {
		t.Fatalf("CollegeService.List: len=%d, want 1", len(list))
	}
	if list, _ := svc.List("北京"); len(list) != 0 {
...
func TestCooperationService_CRUD(t *testing.T) {
	...
	cp, err := svc.Create("校企共建", "col-1", "ent-1", "joint_lab", "描述", start, end, 30)
	if err != nil {
...
func TestRescueCaseService_Get(t *testing.T) {
	...
	rc, err := svc.Create("救援案例", "山火", "重庆", "M300", "应急队", "摘要", "结果", "教训", "协会", time.Now())
	if err != nil {
		t.Fatalf("RescueCaseService.Create: %v", err)
	}
	got, err := svc.Get(rc.ID)
...
func TestEmergencyDeptService_CRUD(t *testing.T) {
	...
	d, err := svc.CreateDept("消防支队", "fire", "重庆", "张三", "138", "http://protocol")
	if err != nil {
	...
	if depts, _ := svc.ListDepts(); len(depts) != 1 {
		t.Fatalf("EmergencyDeptService.ListDepts: len=%d, want 1", len(depts))
	}

	drill, err := svc.CreateDrill(d.ID, "联合演练", "山火", time.Now(), 10, 5, "成功")
	if err != nil {
		t.Fatalf("EmergencyDeptService.CreateDrill: %v", err)
	}
	if drills, _ := svc.ListDrills(d.ID); len(drills) != 1 {
		t.Fatalf("EmergencyDeptService.ListDrills: len=%d, want 1", len(drills))
	}
	if drills, _ := svc.ListDrills("nope"); len(drills) != 0 {
...
func TestAssociationMemberService_Roles(t *testing.T) {
	...
	m, err := svc.AddMember("user-1", "ent-1", domain.AssocMember)
	if err != nil {
...
func TestCompetitionService_DeleteUpdateRegister(t *testing.T) {
	...
	c, err := svc.Create(domain.Competition{Title: "竞速赛", Category: "racing"})
	if err != nil {
		t.Fatalf("CompetitionService.Create: %v", err)
	}
	// Register 不存在的赛事 → 报错
	if _, err := svc.Register("nope", "user-1", "队", 1, "138", "张三", "138", "id", "", ""); err == nil {
		t.Fatal("CompetitionService.Register: expected error for unknown competition")
	}
	// Update 不存在的赛事 → 报错
	if _, err := svc.Update(domain.Competition{ID: "nope"}); err == nil {
...
func TestEventService_DeleteUpdateRegister(t *testing.T) {
	...
	e, err := svc.Create("产业论坛", "forum", "描述", "地点", "", start, end, 100)
	if err != nil {
		t.Fatalf("EventService.Create: %v", err)
	}
	// Register 不存在的活动 → 报错
	if _, err := svc.Register("nope", "user-1", "张三", "138", "公司"); err == nil {
		t.Fatal("EventService.Register: expected error for unknown event")
	}
	// Update 不存在的活动 → 报错
	if _, err := svc.Update("nope", "t", "", "", "", "", "", start, end, 10); err == nil {
...
func TestResourceService_DeleteBookAndVisibility(t *testing.T) {
	...
	r, err := svc.Create("owner-1", "无人机", "drone", "M300", "规格", "地点", "预约信息", 10000, "")
	if err != nil {
	...
	if _, err := svc.Book("user-1", "nope", "2026-01-01", "用途", "张三", "138"); err == nil {
...
func TestEmergencyService_FullCRUD(t *testing.T) {
	...
	r, err := svc.CreateResource("owner-1", "应急无人机", "drone", "M300", "南岸", "138", 2)
	if err != nil {
		t.Fatalf("EmergencyService.CreateResource: %v", err)
	}

	// UpdateResource
	up, err := svc.UpdateResource(r.ID, "应急无人机2", "无人机", "M300RTK", "渝中", "139", "engaged", 5)
...
func TestAchievementService_UpdateError(t *testing.T) {
	...
	if _, err := svc.Update("nope", "t", "", "", "", "", "", nil, nil); err == nil {
...
func TestRDChallengeService_DeleteUpdate(t *testing.T) {
	...
	c, err := svc.Create("ent-1", "长续航电池", "电池", "描述", 500000, time.Now().AddDate(0, 3, 0))
	if err != nil {
		t.Fatalf("RDChallengeService.Create: %v", err)
	}
	if _, err := svc.Update("nope", "", "", "", "", 0, time.Now()); err == nil {
...
func TestResearchProjectService_DeleteUpdate(t *testing.T) {
	...
	p, err := svc.Create("航路规划", "空域", "描述", "重庆大学", "Q1调研", []string{"巡航科技"}, 200000, time.Now(), time.Now().AddDate(1, 0, 0))
	if err != nil {
		t.Fatalf("ResearchProjectService.Create: %v", err)
	}
	if _, err := svc.Update("nope", "", "", "", "", "", "", nil, 0, time.Now(), time.Now()); err == nil {
...
func TestWorkOrder_AcceptIntentErrors(t *testing.T) {
	...
	// 需求不存在
	if _, err := orderSvc.AcceptIntent(pub, "nope", "intent-1", 0); err == nil {
		t.Fatal("AcceptIntent: expected error for unknown demand")
	}
	// 需求未发布（pending）
	pd, _ := demandSvc.Create(pub, service.CreateDemandInput{PublisherName: "企业", Contact: "138", Title: "待审核需求"})
	if _, err := orderSvc.AcceptIntent(pub, pd.ID, "intent-1", 0); err == nil {
		t.Fatal("AcceptIntent: expected error for unpublished demand")
	}
	// 正常场景 + 重复确认报错
	d := publishDemandForTest(t, demandSvc, pub, "正常需求")
	it, err := intentSvc.Create(worker, d.ID, service.CreateIntentInput{IntentorName: "飞手", Contact: "139"})
	if err != nil {
		t.Fatalf("IntentService.Create: %v", err)
	}
	wo, err := orderSvc.AcceptIntent(pub, d.ID, it.ID, 100000)
	if err != nil || wo.Status != domain.WorkOrderPending {
		t.Fatalf("AcceptIntent: status=%q err=%v", wo.Status, err)
	}
	// 意向已被确认 → 重复确认报错
	if _, err := orderSvc.AcceptIntent(pub, d.ID, it.ID, 200000); err == nil {
...
func TestWorkOrder_StatusPreconditions(t *testing.T) {
	...
	it, _ := intentSvc.Create(worker, d.ID, service.CreateIntentInput{IntentorName: "飞手", Contact: "139"})
	wo, _ := orderSvc.AcceptIntent(pub, d.ID, it.ID, 100000)

	// StartWork：订单不存在 / 非飞手
	if _, err := orderSvc.StartWork(worker, "nope"); err == nil {
		t.Fatal("StartWork: expected error for unknown order")
	}
	if _, err := orderSvc.StartWork(pub, wo.ID); err == nil {
		t.Fatal("StartWork: expected error when publisher (non-worker) starts")
	}
	// CompleteWork：订单不存在
	if _, err := orderSvc.CompleteWork(worker, "nope", nil); err == nil {
		t.Fatal("CompleteWork: expected error for unknown order")
	}
	// CompleteWork：pending 状态不允许（状态前置校验）/ 非飞手
	if _, err := orderSvc.CompleteWork(worker, wo.ID, nil); err == nil {
		t.Fatal("CompleteWork: expected error when order not ongoing")
	}
	if _, err := orderSvc.CompleteWork(pub, wo.ID, nil); err == nil {
		t.Fatal("CompleteWork: expected error when non-worker completes")
	}
	// AcceptCompletion：pending 状态不允许
	if _, err := orderSvc.AcceptCompletion(pub, wo.ID); err == nil {
		t.Fatal("AcceptCompletion: expected error when order not awaiting_accept")
	}
	// RequestRework：pending 状态不允许
	if _, err := orderSvc.RequestRework(pub, wo.ID, "note"); err == nil {
		t.Fatal("RequestRework: expected error when order not awaiting_accept")
	}

	// 推进到 awaiting_accept
	wo, _ = orderSvc.StartWork(worker, wo.ID)
	wo, _ = orderSvc.CompleteWork(worker, wo.ID, []string{"a.jpg"})
...
func TestWorkOrder_RejectIntentErrors(t *testing.T) {
	...
	// 需求不存在
	if err := orderSvc.RejectIntent(pub, "nope", "intent-1"); err == nil {
		t.Fatal("RejectIntent: expected error for unknown demand")
	}
	d := publishDemandForTest(t, demandSvc, pub, "拒绝意向需求")
	it, _ := intentSvc.Create(worker, d.ID, service.CreateIntentInput{IntentorName: "飞手", Contact: "139"})
	// 非发布者
	if err := orderSvc.RejectIntent(worker, d.ID, it.ID); err == nil {
		t.Fatal("RejectIntent: expected error for non-publisher")
	}
	// 意向不存在
	if err := orderSvc.RejectIntent(pub, d.ID, "nope"); err == nil {
		t.Fatal("RejectIntent: expected error for unknown intent")
	}
	// 正常拒绝
	if err := orderSvc.RejectIntent(pub, d.ID, it.ID); err != nil {
...
func TestWorkOrder_FindByID(t *testing.T) {
	...
	if _, err := orderSvc.FindByID(pub, wo.ID); err != nil {
		t.Fatalf("FindByID(publisher): %v", err)
	}
	if _, err := orderSvc.FindByID(worker, wo.ID); err != nil {
		t.Fatalf("FindByID(worker): %v", err)
	}
	if _, err := orderSvc.FindByID(domain.Actor{ID: "stranger", Role: domain.RoleIndividual}, wo.ID); err == nil {
		t.Fatal("FindByID: expected error for non-party user")
	}
	if _, err := orderSvc.FindByID(pub, "nope"); err == nil {
		t.Fatal("FindByID: expected error for unknown order")
	}

	// RequestCancel：非订单双方
	if _, err := orderSvc.RequestCancel(domain.Actor{ID: "stranger", Role: domain.RoleIndividual}, wo.ID, "reason"); err == nil {
...
func TestIntentService_CreateErrors(t *testing.T) {
	...
	// 空 demand_id
	if _, err := intentSvc.Create(worker, "", service.CreateIntentInput{Contact: "138"}); err == nil {
		t.Fatal("IntentService.Create: expected error for empty demand_id")
	}
	// 空 contact
	if _, err := intentSvc.Create(worker, "d", service.CreateIntentInput{Contact: ""}); err == nil {
		t.Fatal("IntentService.Create: expected error for empty contact")
	}
	// 需求不存在
	if _, err := intentSvc.Create(worker, "nope", service.CreateIntentInput{Contact: "138"}); err == nil {
		t.Fatal("IntentService.Create: expected error for unknown demand")
	}
	// 需求未发布
	pd, _ := demandSvc.Create(pub, service.CreateDemandInput{PublisherName: "企业", Contact: "138", Title: "待审核"})
	if _, err := intentSvc.Create(worker, pd.ID, service.CreateIntentInput{Contact: "138"}); err == nil {
		t.Fatal("IntentService.Create: expected error for unpublished demand")
	}
	// 自己发布的需求
	d := publishDemandForTest(t, demandSvc, pub, "已发布需求")
	if _, err := intentSvc.Create(pub, d.ID, service.CreateIntentInput{Contact: "138"}); err == nil {
		t.Fatal("IntentService.Create: expected error for self intent")
	}
	// 正常创建（IntentorName 为空 → 默认 a.ID）
	it, err := intentSvc.Create(worker, d.ID, service.CreateIntentInput{Contact: "138"})
	if err != nil {
		t.Fatalf("IntentService.Create: %v", err)
	}
	...
	// 重复 pending 意向
	if _, err := intentSvc.Create(worker, d.ID, service.CreateIntentInput{Contact: "138"}); err == nil {
...
func TestIntentService_ListByDemandListMine(t *testing.T) {
	...
	if _, err := intentSvc.Create(worker, d.ID, service.CreateIntentInput{IntentorName: "飞手", Contact: "139"}); err != nil {
		t.Fatalf("IntentService.Create: %v", err)
	}

	// 非发布者/非管理员 → 报错
	if _, err := intentSvc.ListByDemand(worker, d.ID); err == nil {
		t.Fatal("ListByDemand: expected error for non-publisher non-admin")
	}
	// 发布者可见
	if list, err := intentSvc.ListByDemand(pub, d.ID); err != nil || len(list) != 1 {
		t.Fatalf("ListByDemand(publisher): len=%d err=%v", len(list), err)
	}
	// 管理员可见
	if list, err := intentSvc.ListByDemand(domain.Actor{ID: "admin", Role: domain.RolePlatformAdmin}, d.ID); err != nil || len(list) != 1 {
		t.Fatalf("ListByDemand(admin): len=%d err=%v", len(list), err)
	}
	// 需求不存在
	if _, err := intentSvc.ListByDemand(pub, "nope"); err == nil {
		t.Fatal("ListByDemand: expected error for unknown demand")
	}
	// ListMine
	if mine, err := intentSvc.ListMine(worker); err != nil || len(mine) != 1 {
		t.Fatalf("ListMine: len=%d err=%v", len(mine), err)
	}
	if mine, _ := intentSvc.ListMine(pub); len(mine) != 0 {
...
func TestMatchingService_ScoreBranches(t *testing.T) {
	...
	results, err := svc.Recommend("user-1", 29.5, 106.5, "cable_inspection", "渝北", 10)
	if err != nil {
	...
	// limit 截断
	limited, _ := svc.Recommend("user-1", 29.5, 106.5, "cable_inspection", "渝北", 2)
	if len(limited) != 2 {
		t.Fatalf("Recommend(limit=2): len=%d, want 2", len(limited))
	}
	// limit 0 → 不截断
	all, _ := svc.Recommend("user-1", 29.5, 106.5, "cable_inspection", "渝北", 0)
	if len(all) != 3 {
...
func TestMatchingService_SearchAndMatch_Branches(t *testing.T) {
	...
	// 无命中关键词
	none, err := svc.SearchAndMatch("不存在", 0, 0, "", 10)
