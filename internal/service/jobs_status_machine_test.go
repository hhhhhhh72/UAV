...
func TestUpdateJobStatusWhitelist(t *testing.T) {
	svc := newJobSvc()
	j, err := svc.CreateJob(entActor(), "飞手招聘", "描述", "重庆", 100000)
	if err != nil {
		t.Fatal(err)
	}
	for _, s := range []string{"draft", "published", "closed"} {
		if _, err := svc.UpdateJob(j.ID, "飞手招聘", "描述", "重庆", "全职", 100000, s); err != nil {
			t.Errorf("UpdateJob(%q): %v, want ok", s, err)
		}
	}
	for _, s := range []string{"", "banana", "active", "OPEN"} {
		if _, err := svc.UpdateJob(j.ID, "飞手招聘", "描述", "重庆", "全职", 100000, s); !errors.Is(err, service.ErrInvalidJobStatus) {
...
func TestPublishCloseStateGuards(t *testing.T) {
	...
	// 草稿不能关闭
	if _, err := svc.CloseJob(entActor(), j.ID); !errors.Is(err, service.ErrInvalidJobTransition) {
	...
	// 重复发布拒绝
	if _, err := svc.PublishJob(entActor(), j.ID); !errors.Is(err, service.ErrInvalidJobTransition) {
		t.Fatalf("re-publish: err=%v, want ErrInvalidJobTransition", err)
	}
	// published → closed
	if _, err := svc.CloseJob(entActor(), j.ID); err != nil {
...
func TestApplyRejectsNonPublishedJobs(t *testing.T) {
	svc := newJobSvc()
	draft, _ := svc.CreateJob(entActor(), "草稿职位", "描述", "重庆", 100000)
	r, _ := svc.CreateResume(indActor(), "简历", "张三", "138", "a@b.com", "本科", "", nil, "", "", "public")

	// 草稿不可投递
	if _, err := svc.Apply(indActor(), draft.ID, r.ID); err == nil {
		t.Fatal("apply to draft job accepted")
	}
	// 招聘中可投递
	if _, err := svc.PublishJob(entActor(), draft.ID); err != nil {
		t.Fatal(err)
	}
	if _, err := svc.Apply(indActor(), draft.ID, r.ID); err != nil {
	...
	// 关闭后新投递被拒
	svc.CloseJob(entActor(), draft.ID)
	if _, err := svc.Apply(indActor(), draft.ID, r.ID); err == nil {
