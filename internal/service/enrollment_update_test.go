...
func TestEnrollmentUpdate(t *testing.T) {
	...
	created, err := svc.Enroll("u-1", "crs-1", service.EnrollmentForm{Name: "张三", Phone: "13800000000", Birthday: "2000-01-02"})
	if err != nil {
	...
	t.Run("admin can update basic info and status", func(t *testing.T) {
		upd, err := svc.Update(admActor(), domain.Enrollment{
	...
	t.Run("non-admin is rejected", func(t *testing.T) {
		_, err := svc.Update(domain.Actor{ID: "u-1", Role: domain.RoleIndividual}, domain.Enrollment{ID: created.ID, Status: "pending"})
	...
	t.Run("invalid status value is rejected", func(t *testing.T) {
		_, err := svc.Update(admActor(), domain.Enrollment{ID: created.ID, Status: "bogus"})
	...
	t.Run("cannot roll back paid/enrolled to pending or rejected", func(t *testing.T) {
		...
		_, err := svc.Update(admActor(), domain.Enrollment{ID: created.ID, Status: "pending"})
		if err == nil || !strings.Contains(err.Error(), "cannot change") {
			t.Fatalf("want rollback error, got %v", err)
		}
		_, err = svc.Update(admActor(), domain.Enrollment{ID: created.ID, Status: "rejected"})
		if err == nil || !strings.Contains(err.Error(), "cannot change") {
			t.Fatalf("want rollback error for rejected, got %v", err)
		}
		// paid → enrolled 为前向流转，允许
		if _, err := svc.Update(admActor(), domain.Enrollment{ID: created.ID, Status: "enrolled"}); err != nil {
			t.Fatalf("paid -> enrolled should be allowed: %v", err)
		}
	})

	t.Run("unknown id returns not found", func(t *testing.T) {
		_, err := svc.Update(admActor(), domain.Enrollment{ID: "enroll-missing", Status: "pending"})
