...
func TestStats(t *testing.T) {
	...
	_ = active
	if expired < 2 {
		t.Logf("stats: total=%d active=%d expired=%d", total, active, expired)
	}
}

func TestGlobal(t *testing.T) {
	if cache.Global == nil {
		t.Fatal("global cache is nil")
	}
	cache.Global.Set("test", 42)
	v, _ := cache.Global.Get("test")
	if v != 42 {
		t.Fatal("global cache")
	}
}
