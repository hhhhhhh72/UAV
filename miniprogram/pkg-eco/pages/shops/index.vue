...
const pickCat = (id) => {
  if (id === 'train') return uni.navigateTo({ url: '/pkg-talent/pages/training/courses' })
  if (id === 'sale' || id === 'parts') return uni.navigateTo({ url: '/pkg-eco/pages/mall/index' })
  uni.showToast({ title: '该分类商家请联系协会推荐', icon: 'none' })
}
const goJoin = () => uni.navigateTo({ url:'/pkg-eco/pages/enterprise/register' })
const callShop = (s) => {
