<template>
  <view class="page" :class="{ 'no-motion': noMotion }" :style="{ paddingTop: (statusBarHeight + 44) + 'px' }" @tap="closeAll">
    <u-nav-bar title="课题攻关" show-back :fixed="true" @back="goBack" />

    <!-- 固定头部：搜索 + 筛选器 + 展开面板（一体吸顶，与 challenges/list 同构） -->
    <view class="sticky-head" :style="{ top: (statusBarHeight + 44) + 'px' }" @tap.stop>

      <!-- 搜索框（与 challenges/list 同款：小红书风搜索按钮） -->
      <view class="sbar">
        <view class="b-search">
          <image class="b-search-ic" src="/static/home/icons/search.svg" mode="aspectFit" />
          <input class="b-sinp" v-model="q" placeholder="搜索课题、关键词" placeholder-class="b-ph" confirm-type="search" @input="onSearch" />
    ...
    <!-- 信息行：共 N 项 + 排序 -->
    <view class="ir">
      <text>共 <text class="irn">{{ filteredAll.length }}</text> 项课题</text>
      <view class="irs-wrap">
    ...
    <view v-else-if="!list.length" class="st">
      <u-empty description="暂无匹配课题">
        <text class="sth">试试调整筛选条件或搜索关键词</text>
        <view class="stb" @tap="resetAll">清除筛选</view>
      </u-empty>
    </view>

    <!-- 列表：纯文字卡片（后端不支持图片；左缘领域色条 + tag 为视觉锚点，与 challenges/list 同构） -->
    <view v-else class="cl" :class="{ replay }">
      <view v-for="x in list" :key="x.id" class="card" :data-id="x.id" :class="{ seen: seen.has(x.id) }" hover-class="tap-scale" hover-start-time="0" hover-stay-time="120" @tap="goDetail(x)">
        ...
        <view class="c-top">
          ...
          <view class="c-budget"><text class="lb">经费</text><text class="vl" :class="{ face: x.budgetText === '面议' }">{{ x.budgetText }}</text></view>
        </view>
        <text class="ct">{{ x.t }}</text>
        <text v-if="x.d" class="c-desc">{{ x.d }}</text>
        <view class="c-meta">
          <text v-if="x.lead" class="c-lead">牵头</text>
          <text class="c-org">{{ x.lead || x.pubLine }}</text>
          <text class="c-dot">·</text>
          <text class="c-dl" :class="{ hot: x.urgent }">{{ x.dl }}</text>
        </view>
        <text v-if="x.pn" class="c-joined">{{ x.pn }}家参与单位</text>
      </view>
    </view>
...
const PAGE_SIZE = 100
const MAX_PAGES = 10
const RENDER_STEP = 100 // DOM 渲染上限步长：数据全量入 fullList，渲染切片逐段揭示（与 challenges/list 同套）
...
/* 领域标签配色：文字卡片无图片时的视觉锚点（与 challenges/list 同套；
   前 6 个领域沿用统一色板，新增技术标准/集群协同按同规则扩展；对比度 ≥4.5:1） */
const FIELD_TAG = {
  ...
  '技术标准': { tagC: '#37474F', tagBg: '#E9EEF0' },
  '集群协同': { tagC: '#263238', tagBg: '#E7EBED' },
}
const FIELD_TAG_DEFAULT = { tagC: '#344054', tagBg: '#EEF1F4' }
/* 课题阶段：规划中(橙) / 招募中(绿) / 进行中(蓝) / 已完成(灰)；招募或进行中且截止 ≤7 天 → 紧急(红) */
const PHASE = {
  planning: { label: '规划中', cls: 'planning' },
  recruiting: { label: '招募中', cls: 'recruiting' },
  progress: { label: '进行中', cls: 'progress' },
...
const statusOf = (it) => {
  ...
  // 已完成不再因截止期变化；招募/进行中 且截止 ≤7 天 → 紧急红标（与 challenges 语义一致）
  const urgent = (s === 'recruiting' || s === 'progress') && dl != null && dl <= 7
  if (urgent) return { label: '紧急', cls: 'urgent', raw: s, urgent: true }
  return { label: ph.label, cls: ph.cls, raw: s, urgent: false }
}
const fmtMoney = (wan) => {
...
const mapItem = (it) => {
  ...
  // 参与单位计数含牵头单位（与 detail 04 芯片 / 设计稿语义一致：牵头 + 成员去重后总数）
  const pn = members.length + (lead && !members.includes(lead) ? 1 : 0)
  const ddl = it.end_date || ''
  // 截止文案：已过期 → 已截止；无截止 → 待定；否则 X 天后截止
  let dlText = '截止待定'
  if (ddl && new Date(ddl).getTime() < Date.now()) dlText = '已截止'
  else if (dl != null) dlText = dl + ' 天后截止'
  return {
    id: it.id,
    t: it.title || '未命名课题',
    f,
    ...
    lead,
    pn,
    pubLine: '发布于 ' + (it.created_at || '').slice(5, 10),
...
const applyFilter = () => {
  ...
  if (selStatus.value) items = items.filter((x) => x.stRaw === selStatus.value) // 按原始阶段过滤（紧急是派生态）
  if (quick.value === '7d') {
  ...
  if (sort.value === 'budget') items.sort((a, b) => b.budget - a.budget)
    ...
    // 已完成/已截止排最后，其余按截止时间升序（越近越靠前）
    items.sort((a, b) => {
      const ac = a.stRaw === 'completed' || a.dl === '已截止' ? 1 : 0
      const bc = b.stRaw === 'completed' || b.dl === '已截止' ? 1 : 0
...
const goDetail = (x) => {
  uni.navigateTo({ url: '/pkg-eco/pages/projects/detail?id=' + encodeURIComponent(x.id) })
}
...
let shownOnce = false
onShow(() => {
  // onLoad 先于首个 onShow 触发，用 flag 去重首次；从其他页返回时静默刷新
...
/* ===== 搜索框：白上白——纯白填充 + 灰描边 + 极淡灰投影，从白底上"浮"起（与 challenges/list 同款） ===== */
.sbar {
...
/* ===== Banner（与 challenges/list 同款：品牌深空蓝渐变；发布课题页开发后接入点击跳转） ===== */
.banner {
  margin: 12px 14px;
  padding: 16px;
  border-radius: 10px;
...
/* 课题阶段徽章：规划中(橙) / 招募中(绿) / 进行中(蓝) / 已完成(灰) / 紧急(红)；全部 ≥4.5:1 */
.c-st.st-planning { color: #C2410C; background: #FFF0E6; }
.c-st.st-recruiting { color: #0B6B41; background: #E9F7F0; }
.c-st.st-progress { color: #074D92; background: #EAF3FB; }
.c-st.st-completed { color: #5D6B82; background: #EEF1F4; }
...
.c-org { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; } /* 牵头单位长文案省略号 */
.c-dot { color: #DDE1E6; }
...
.c-budget .vl.face { font-size: 13px; font-weight: 600; color: #667085; } /* 面议非数字：不用金额的重量渲染，避免误读为最高经费 */
...
.p-chip {
  min-height: 36px; /* 触控目标：30px→36px（与筛选 pill 同档） */
  padding: 0 13px;
  border: 1px solid #E4E7EC;
  border-radius: 6px;
  background: #fff;
...
.slider-range {
  ...
  border-radius: 2px;
  background: #0A66C2;
  transition: background .2s ease; /* 面议档位切换时颜色平滑过渡；拖动由内联 left/width 驱动，不加位置过渡防拖拽延迟——拖动跟手 1:1 无缓动，松手即定格 */
}
...
.thumb {
  ...
  transition: border-color .2s ease; /* 面议禁用时描边色平滑过渡 */
}
.slider-labels {
  display: flex;
  justify-content: space-between;
...
.money-inputs.disabled { opacity: 0.5; pointer-events: none; transition: opacity .2s ease; } /* 面议禁用时透明度平滑过渡 */
...
.bt {
  ...
  align-items: center;
  justify-content: center;
  z-index: 35; /* 低于 sticky-head(40)——修复筛选面板展开时返回顶部按钮浮层穿透抢点击；仍高于蒙层(30)与页面内容 */
  opacity: 0;
  transform: scale(0.5);
  pointer-events: none;
...
/* ===================== 动效规范（对齐全局动画规范，与 challenges/list 同套） =====================
