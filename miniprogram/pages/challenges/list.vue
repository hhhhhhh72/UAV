<template>
  <view class="page">
    ...
    <!-- Info Row -->
    <view class="ir">
      <text>共 <text class="irn">{{ shown }}</text> 项难题</text>
      ...
      <view v-for="x in list" :key="x.id" class="card" @tap="goDetail(x)">
        <view class="chd">
          <view class="ctags">
            ...
            <text class="cs2" :class="stCls(x.st)">{{ stLabel(x.st) }}</text>
          </view>
        </view>
        ...
        <view class="cft">
          <view class="cm">
            <text class="co">发布于 {{ x.dt }}</text>
          </view>
          <view class="cr">
            <text class="crw">{{ fmtMoney(x.budget) }}</text>
...
const ST = { open: '进行中', urgent: '紧急', closed: '已截止' }
...
const fc = (f) => FC[f] || '#666'
const stLabel = (s) => ST[s] || s || '进行中'
const stCls = (s) => (ST[s] ? s : 'open')
...
const dlOf = (d) => {
  ...
  return { text: dl + ' 天后截止', urgent: dl <= 7 }
}

// 后端字段映射为卡片字段（缺失字段优雅降级）
const mapItem = (it) => {
  const dl = dlOf(it.deadline)
  return {
    id: it.id,
    t: it.title || '未命名难题',
    f: it.field || '其他',
...
const applyFilter = () => {
  ...
  if (sort.value === 'reward') items.sort((a, b) => (b.budget || 0) - (a.budget || 0))
...
const fetchMore = async () => {
  ...
  try {
    const res = await request({ url: '/api/v1/rd-challenges', data: { page: nextPage + 1, page_size: PAGE_SIZE } })
    const items = Array.isArray(res) ? res : (res?.items || [])
...
.sbar { display: flex; align-items: center; gap: 10px; padding: 8px 14px 12px; background: #fff; }
...
/* ===== Tabs ===== */
.tabs { display: flex; padding: 6px 14px 8px; background: #fff; white-space: nowrap; }
.tab { flex-shrink: 0; padding: 8px 16px; font-size: 13px; color: #666; position: relative; }
.tab.active { color: var(--color-primary); font-weight: 600 }
.tab.active::after { content: ''; position: absolute; bottom: -8px; left: 50%; transform: translateX(-50%); width: 20px; height: 3px; background: var(--color-primary); border-radius: 2px }

/* ===== Banner ===== */
.banner { margin: 12px 14px; padding: 16px; border-radius: 8px; background: #1a237e; display: flex; align-items: center; gap: 12px; color: #fff; position: relative; overflow: hidden; opacity: 0; animation: cardIn .4s ease .1s forwards; }
...
.banner-btn { flex-shrink: 0; padding: 7px 16px; border-radius: 8px; background: rgba(255, 255, 255, .2); color: #fff; font-size: 12px; font-weight: 500; position: relative; z-index: 1; }
...
.cs2.open { background: #e8f5e9; color: #2e7d32 }
.cs2.urgent { background: #fce4ec; color: #c62828 }
.cs2.closed { background: #f5f5f5; color: #999 }
...
.co { font-size: 11px; color: #666; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 140px; }
...
.cdl.hot { color: #c62828; font-weight: 500 }
