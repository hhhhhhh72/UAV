...
| 数据库 | PostgreSQL 16（生产） / 内存存储（开发），**92 张存活表**（96 次 CREATE TABLE 减去被 drop 的 shops/demand_bids/association_members；**120 组迁移，240 个 SQL 文件**） |
...
├── migrations/                    # 120 组迁移（240 个 SQL 文件，92 张存活表）
