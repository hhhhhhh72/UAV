...
| **证书到期只能靠人去腾讯云控制台重新下载**（2026-09-21，用户「按顺序来 除了1」的第 4 项） | 证书 11-08 到期，**没有任何自动续期**：`certbot certificates` 报「No certificates found」，`/etc/nginx/certs/api.cqnarc.cn.fullchain.crt` 是 Aug 10 手工放进去的（同日还有一张 `uavtest.cloud.crt`，签发者 `TrustAsia DV TLS RSA CA 2024` 正是腾讯云免费证书的 CA —— 两张都是控制台下载后手工装的），crontab 里零条证书任务。**动手前先试标准解 HTTP-01，被 Let's Encrypt 直接挡回**：`Detail: 43.174.225.201: Invalid response from https://dnspod.qcloud.com/static/webblock.html?d=api.cqnarc.cn` —— 根因是**域名未做 ICP 备案**，腾讯云把**境外来源的 80 端口**劫持到备案提示页，而 LE 的多视角校验节点全在境外。同一境外出口只换端口实测：`http://api.cqnarc.cn/` → 302 到 dnspod 拦截页，`https://api.cqnarc.cn/ops-status.json` → 200 真实内容，**443 没被劫持**。certbot 2.9 的 nginx 插件与 standalone 插件源码 `supported_challenges` 都只返回 `[challenges.HTTP01]`（实测配 `--preferred-challenges tls-alpn-01` 一律 "None of the preferred challenges are supported"）；lego 能做 ALPN 但**强制要 `--email`**（用户明确说「不用邮箱，跟报错报告一样发企业微信就行」） | **已修复**：改用 `acme.sh`（v3.1.6 单文件脚本，哈希 `c7d68b…8d7` 钉死在 `deploy/cert-renew.sh` 里，不符即拒绝执行）+ **TLS-ALPN-01**（在 443 上校验，不需要邮箱、不需要 DNS API）。`deploy/cert-renew.sh` 每天 04:30 由 cron 跑：先看证书还剩几天，**没到 30 天阈值就直接退出、一次也不碰 nginx**；到期才 `cert-hooks.sh pre`（停 nginx 让出 443）→ acme.sh → `post`（拉回 nginx）。**实测一次续期停机 24–30 秒**，逐帧看日志发现 LE 3 秒内就校验完了、剩下 25 秒全是 acme.sh 自己的轮询下载 —— 所以 `pre` 在停服务**之前**先挂一个 300 秒**死人开关**（`setsid`+`nohup`，显式 `9>&-`），即使被 SIGKILL、`post` 根本没跑，站点也会自己活过来。**配套**：`ops-status.sh` 证书一节改成读 **nginx 实际在服务的那张证书**（`openssl s_client` 打 127.0.0.1:443）并与文件 serial 比对 —— 只看文件的话「换了证书但没 reload」是全绿假绿，而续期脚本每天都在写那个文件；`jobs` 加续期日志心跳（26h）；`certbot.timer` 已 disable（没有任何 lineage，只会每天两次空转，留着反而让人以为证书归 certbot 管）。**演练**：`verify-cert-renew.sh` 9 项 + `verify-cert-hooks.sh` 9 项 + `verify-ops-status.sh` 15 项 + `verify-alerts.sh` 13 项，全绿。**演练当场揪出两个真 bug**：① 续期后读不到服务端证书时 `after_serial` 是空串，而空串 ≠ 旧 serial 会被判成「换成功了」→ 报假绿，已加空值拦截；② 死人开关**继承了 flock 的 fd 9**，于是续期结束后 5 分钟内再跑都被判成「已有进程在跑」而静默跳过。**顺带查明**：境外访问 `http://` 会看到腾讯云备案提示页（`https://` 正常），这是没备案的直接后果。**遗留**：一旦完成 ICP 备案，把 `--alpn` 换回 webroot HTTP-01 即可（80 端口那段 `/.well-known/acme-challenge/` 一直留着），那时连这 25 秒停机都不需要；25 秒也能靠 nginx `stream`+`ssl_preread` 按 ALPN 分流做到零停机，但那要把 443 整个挪到内网口，结构性改动不值当 |

| **商城订单评价完后状态还是「待评价」**（2026-09-22 用户报） | 生产库里评价记录明明在（`reviews` 有 `target_type='order'`、`target_id=torder-1790…`、5 星「非常可以」），订单列表却仍显示「待评价」。根因是**服务端根本没有「已评价」这个事实**：① `trade_orders` 只有 `pending/paid/shipped/completed` 状态机、没有 `reviewed`，而 `completed` 在小程序展示层被映射成「待评价」（`orderAdapter.js:27`）；② `POST /api/v1/reviews` 对 `target_type='order'` **零校验、也不回写订单** —— `service/reviews_resources.go` 里只有 `work_order` 一个校验分支（那次「P0 死链修复」只补了工单）；③ 「已评价」只写在手机本地存储 `order_reviewed_prod` 里，换设备/清缓存、或在微信开发者工具里评价而用真机查看，就立刻退回「待评价」；④ 「待评价」列表的筛选只看 `o.status==='completed'`、**不看已评价标记**，而角标统计却把已评价的减掉了 —— 角标消失、订单还留在列表里，两边口径不一致 | **已修复**：① `domain.TradeOrder` 新增 `Reviewed`（响应增强字段，不入库），由 `ReviewRepository.ListReviewedTargetIDs` 一次 `ANY($3)` 批量查推导（口径与 Submit 幂等一致：pending/approved 算、**rejected 不算**），`TradeOrderService.ListMine` 填充；**刻意不加 `trade_orders.reviewed` 冗余列** —— 同一个事实记两处迟早对不上（`reg_count` 漂移是前车之鉴）。② 补 `order` 评价校验：订单存在 + **是买家本人** + 状态 `completed`，新增哨兵 `ErrReviewTargetNotFound/NotAllowed/NotReady` → Handler 映射 **404/403/409**（此前一律 500），工单那三条一并换成哨兵。③ 小程序 `orderAdapter.js` 的 `applyReviewed`、角标、「待评价」列表筛选三处统一以服务端 `reviewed` 为准，本地存储降级为兜底。**验证**：新增 `review_order_test.go`（9 条断言，含「别人评价过不影响我的 reviewed」「被驳回后可重评」）；生产上直接打接口核对 —— 他那 6 单里 `reviewed=true` 恰好落在真实评价过的 2 单上，伪造订单 404、非买家 403、重复评价 409。**踩坑**：`review_dedup_test.go` 原本拿 `target_type='order'` + 不存在的 `torder-dup-1` 做查重夹具，补了前置校验后会被 404 先挡掉，夹具换成 `enterprise`（查重逻辑与 target 类型无关）。**注意**：后端已发布，但**小程序要重新编译上传**才会读这个字段。**同一天第二次报才定位到真问题**：用户说的「待评价」是**卖家视角**的误报 —— 他是**发布方（鸟咪，user-1789026379704722059）**，买家（A）评价了他卖出的商品；而「待评价」这个说法只对**买家**成立（卖家根本没有评价入口），且 `reviewed` 算的是"**当前用户**有没有评价过"，卖家恒为 false → 他那单**永远**出不去「待评价」，卡片写「待评价」而按钮写「已完成」，自相矛盾。修法按角色分流：`completed` 对买家是「待评价」、对**卖家**是「已完成」；「待评价」列表与角标只算买家侧（卖家侧用 counts 里没有的哨兵值 `seller_done` 跳过）；`applyReviewed` 对卖家直接短路。新增 `scripts/check-order-status-role.cjs` 并接进 CI —— 它**跑真正的 orderAdapter.js**（把 request/uni 换成桩 + 构造订单），不是复刻判断逻辑（复刻的话源码改回去它也照样绿）；**变异验证**：摘掉两处角色判断 → 红，且直接报「卖家卖出的 completed 单标签应为「已完成」，实际「待评价」」，还原后 sha256 一致。另外用**线上真实数据**跑真代码做前后对照（抓鸟咪的 `/trade-orders/mine` 喂进 adapter）：修复前「待评价」列表=那条单、角标 1；修复后列表空、角标 0、标签「已完成」。**教训**：用户说「我看到的状态不对」时，先问一句**"我在这单里是什么角色"** —— 我第一轮把后端 `reviewed` 修对了，方向没错，但对这个用户一点用都没有。**紧接着的追问「那我已完成的订单该放在哪里」**又暴露了这个修法的缺口：把卖家的单从「待评价」拿掉、却没给它新家（只在「全部订单」里能看到）。按用户选的 A 方案收口：第 4 个入口的字面由「待评价」改为「**已完成**」，装**我所有的已完成单**（我买的已评价/未评价 + 我卖的成交单）；买家侧没评价的卡片仍写「待评价」并给「去评价」按钮、角标仍只提示未评价的；**入口数量与顺序不动，只改一个字面**。检查器同步加了两条断言：售后结案单不许混进「已完成」、「已完成」角标是提醒（只数买家侧未评价）而不是列表条数 |
