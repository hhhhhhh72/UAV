...
| **日报「发了」但没人确认它送到了**（2026-09-18） | 平台上每天发生了什么，此前只能靠人登录后台翻页面 —— 而人不会天天翻；运维告警只解决「坏了要有人说」，**没坏**的那些天里新注册了几个用户、有没有人发需求、托管里有多少钱，没有任何渠道主动告诉负责人。补上日报脚本之后还有第二层：如果只检查「文件生成了没有」，webhook 被停用/换 key 时脚本照样每天生成、文件天天新鲜，群里却一条都收不到 —— 与备份静默失败、告警假绿、保洁从没跑过同一个病根，只是这次检查的对象是**投递**而不是**生成** | **已修复**：① 新增 `deploy/daily-report.sh`（cron 每天 19:05）—— 一条纯文本消息：当日 00:00→运行时刻的新增/资金/待办/累计/托管 + 系统一句话，全是数字、不含 ID 手机号姓名；当天没有数据也照发（本身就是心跳）。② `daily-report.json` 里的 `delivered_epoch` **只在推送成功后**更新，`ops-status.sh` 的 jobs 一节检查的正是它（阈值 30h ＝ 24h 间隔 ＋ 6h 抖动余量 —— 日任务的阈值必须留余量，否则每天都会出现一段假告警窗口）。③ 把告警与日报共用的通道实现抽成 `deploy/lib-notify.sh`（形状校验 / errcode 校验 / 打码 / 分项清单**只此一份**），并给 `verify-alerts.sh` 添了一项**元检查**：拿生产快照实际输出的分项与清单逐项比对，漏项直接演练失败 —— 把「记得同步两份列表」交给机器。④ 干跑时抓到真 bug：SQL 里写 `CASE WHEN '$DAY'='' THEN ... ELSE ''::date END`，PostgreSQL 生成计划时会折叠常量表达式，`''::date` 哪怕分支走不到也照样报 invalid input syntax —— 表现是「指定 DAY 时正常、每天真正跑的那条路径 psql 一行都不返回」，已改为在 bash 里选好时间窗再拼 SQL |
