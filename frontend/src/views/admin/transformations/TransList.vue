<template>
  <div class="admin-page"><div class="page-header"><h2>成果名称</h2><el-button type="primary" @click="handleAdd">新增</el-button></div>
...
<el-table-column prop="title" label="成果名称" />
      <el-table-column prop="target_org" label="转化方" />
...
const loadData=async()=>{loading.value=true;try{const r=await axios.get('/api/v1/admin/transformations',{params:{...searchForm.value,page:pagination.page,page_size:pagination.pageSize}});tableData.value=r.data.data||[];pagination.total=r.data.total||0}catch{ElMessage.error('load failed')}finally{loading.value=false}}
