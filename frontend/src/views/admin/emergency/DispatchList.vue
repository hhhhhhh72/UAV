<template>
  <div class="admin-page"><div class="page-header"><h2>事件描述</h2><el-button type="primary" @click="handleAdd">新增</el-button></div>
...
<el-table-column prop="event_desc" label="事件描述" />
      ...
      <el-table-column prop="commander" label="指挥" />
...
const loadData=async()=>{loading.value=true;try{const r=await axios.get('/api/v1/admin/emergency-dispatches',{params:{...searchForm.value,page:pagination.page,page_size:pagination.pageSize}});tableData.value=r.data.data||[];pagination.total=r.data.total||0}catch{ElMessage.error('load failed')}finally{loading.value=false}}
