<template>
  <div class="admin-page"><div class="page-header"><h2>持证人</h2><el-button type="primary" @click="handleAdd">新增</el-button></div>
    <DataToolbar v-model="searchForm" :filters="filterConfig" @search="handleSearch" @reset="handleReset" />
    <el-table v-loading="loading" :data="tableData" border stripe>
      <el-table-column prop="id" label="ID" width="140" />
<el-table-column prop="holder_name" label="持证人" />
...
const loadData=async()=>{loading.value=true;try{const r=await axios.get('/api/v1/admin/certs',{params:{...searchForm.value,page:pagination.page,page_size:pagination.pageSize}});tableData.value=r.data.data||[];pagination.total=r.data.total||0}catch{ElMessage.error('load failed')}finally{loading.value=false}}
