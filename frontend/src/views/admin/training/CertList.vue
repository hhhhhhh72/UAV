  ...
  <div class="list-page">
    ...
    <div class="table-wrap">
      <el-table v-loading="loading" :data="listData" row-key="id" stripe border @selection-change="onSelectChange">
        ...
        <el-table-column prop="cert_no" label="证书编号" width="180">
          <template #default="{ row }">
            <span class="cell-mono">{{ row.cert_no || '-' }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="holder_name" label="持有人" min-width="140" />
    ...
    <el-dialog v-model="formVisible" :title="formEdit?'编辑证书':'新增证书'" width="560px" destroy-on-close>
      <el-form :model="form" label-width="90px">
        <el-row :gutter="16">
          <el-col :span="12"><el-form-item label="证书编号" required><el-input v-model="form.cert_no" /></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="证书类型"><el-input v-model="form.cert_type" /></el-form-item></el-col>
        </el-row>
        <el-row :gutter="16">
          <el-col :span="12"><el-form-item label="持有人"><el-input v-model="form.holder_name" /></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="发证机构"><el-input v-model="form.issuer" /></el-form-item></el-col>
        </el-row>
        <el-row :gutter="16">
        ...
        <el-form-item label="状态"><el-select v-model="form.status"><el-option label="有效" value="valid" /><el-option label="已过期" value="expired" /><el-option label="已吊销" value="revoked" /></el-select></el-form-item>
...
const form=reactive({id:'',cert_no:'',holder_name:'',cert_type:'',issuer:'',issue_date:'',expire_date:'',status:'valid',remark:''})
...
const submitForm=async()=>{if(!form.cert_no){ElMessage.warning('请输入证书编号');return};formLoading.value=true;try{const p={...form};formEdit.value?await api.update(form.id,p):await api.create(p);ElMessage.success(formEdit.value?'更新成功':'创建成功');formVisible.value=false;loadData()}catch(e){ElMessage.error(e?.response?.data?.message||'操作失败')}finally{formLoading.value=false}}
