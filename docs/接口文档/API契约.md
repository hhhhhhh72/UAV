...
| **案例** | `GET /cases` `POST/PUT/DELETE /admin/cases` | 案例库。媒体：视频案例传 `video_url`（mp4，≤40MB，小程序详情卡片直接播），图文案例传 `images`（多张，详情里排在视频之后）；两者可并存，展示时视频优先。公开列表 `GET /cases` 只返回 `status=published`（下架/待审不对外可见），管理端可用 `?status=` 筛选 |
